...
/**
 * 档位文案（`ab_set_meal_item.slot`：1–5）· **唯一真源**
 *
 * 服务端（后台编排 / 订单导出）与用户端（小程序展示）**共用本表**。
 * ⚠️ 不要在别处再写一份映射：改一处漏一处，而**没有任何门禁会红**。
 *    （2026-09-21 清理前：端上 `utils/format.ts` 与 `order-admin.service.ts` 各手抄过一份，
 *     `shared-types` 里另有一个取值全仓零使用的 `DishSlot` 枚举 —— 该文件**已整份删除**，
 *     别再去找它；端到端断言也已从「出参提到 `DishSlot`」改为「出参点名两条轴的物理落点」。）
 *
 * ⚠️ **别与「品类轴」混淆**（这是本项目最容易搞错的一处）：
 *    · 本表 = **档位轴**，回答「它在套餐里填哪个坑」→ 数字 `1`–`5`，中文写 **`汤`**；
 *    · 另有 **品类轴** = `ab_dish.category`，回答「这道菜本身是什么」→ 字符串
 *      `main 主荤 / half 半荤 / veg 素菜 / soup 汤品 / staple 主食`，
 *      展示文案见 `@abox/shared-types` 的 `DISH_CATEGORY_LABEL`，中文写 **`汤品`**。
 *    两轴**概念相邻、取值不同、中文不同** ⇒ `汤` 与 `汤品` 并存**不是漂移，勿强行统一**
 *    （统一会改掉菜品库 / 角标 / 溯源页的真实展示口径）。
 */
