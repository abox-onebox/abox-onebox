 ...
 *    故列头直接写「费率（快照）」。
 * 3. **「佣金入账」按钮一期点下去多半是 0 条** —— 佣金在「取餐确认」时即时入账，
 *    `pending` 只是 M4 两步结算上线后的中间态。**服务端会把原因放在 `note` 里**，
  ...
  try {
    await ElMessageBox.confirm(
      `将把 ${scope} 的「待入账」佣金计入团长余额。\n\n` +
        '· 整批单事务：要么全入账、要么全不入账\n' +
        '· 幂等：可安全重复点击，已入账的会跳过\n' +
        '· ⚠️ 一期佣金在「取餐确认」时即时入账，故通常为 0 条，这不是故障',
      '佣金入账（补跑）',
...
<template>
  <div class="page-container">
    ...
    <p class="note">
      ...
      <strong>同一执行口</strong>。一期佣金在取餐确认时即时入账，故该按钮通常返回 0 条 ——
      结果弹窗会把服务端给出的原因原样展示。
    </p>
  </div>
</template>
