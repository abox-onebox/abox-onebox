...
/**
 * 幂等清空：**一次性清空全部表**（`ALL_ENTITIES`，当前 **27** 张），再开始插入。
 *
 * 为什么不能边清边插：
...
async function main(): Promise<void> {
  ...
  await supRepo.save([
    // 出餐型（演示占位名 · C5：不得使用「巡礼之年」）
    {
      ...
      category: '本帮菜',
      // 4 家演示供应商**均为已核验**：出餐链路（S1/S2/S3）要求 canServe=true
      // （合作中 ∧ 资质已通过 ∧ 证照未过期），否则演示环境一进 P21 就被 50001 拦。
  ...
  //
  // ⚠️ **`monthOrders` 是「推导值的落库缓存」，不是手工事实**（缺陷 #94 另注 · 2026-09-18）：
  //    `LeaderPromotionService.audit()` 每次晋级审计都会用 `ab_commission` 的**当月份数统计**
  //    重算并回写它（`PromotionService:141-150`）。故这里的 186 / 78 / 42 / 30 / 8 只在
  //    **刚种完、还没跑过任何计佣审计** 时成立；此后会被真实统计覆盖（例如 e2e 在「本月」
  //    给李明造过单，审计后 `month_orders` 就变成那个数）。这不是数据错误，是「演示值 vs 实算值」。
  //    ⭐ 副作用：`level` 只升不降（C2 审计的语义），于是「首席团长本月 7 单」会同时出现
  //    「生效等级 chief」与「业绩测算 trainee」——**这正是缺陷 #94 的样本**，
  //    也是 P16/P20 必须把这两个概念分开显示的原因（见《接口规范》§4.6 `mine` 三字段）。
