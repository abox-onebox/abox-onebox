/**
 * ⚠️ **未实装 · 占位骨架**（内容只有 `export {}`，全仓 **0 引用**，已机械核对）
 *
 * 本文件来自《项目目录结构 v2.0》的骨架清单。**文件名描述的是「计划」，不是「事实」** ——
 * 勿据此文件判断项目具备该能力。
 *
 * 真实实现：**无**此守卫 —— 真实验签在 `providers/wx-pay/wx-pay.provider.ts#parseNotify`（由 `modules/payment/payment.service.ts` 调用，失败回 `FAIL`）
 *
 * 原骨架说明：common/guards/wx-signature.guard.ts —— 微信回调验签守卫 · 占位骨架
 *
 * 核对方式：`tests/tools/dead-files-scan.mjs`（有效代码 ≤3 行 + 仅 `export {}` + 引用数 0）
 * 复活方式：若确要在此文件落地实现，**先删掉本段头注释**（否则会被继续当空壳扫出来）。
 */
export {};
