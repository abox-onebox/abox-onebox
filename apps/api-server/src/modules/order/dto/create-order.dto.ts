 ...
 * 勿据此文件判断项目具备该能力。
 *
 * 真实实现：`modules/order/dto/order.dto.ts#CreateOrderReqDto`（它 `implements CreateOrderDto`，而该**类型**来自 `@abox/shared-types`）
 *
 * 原骨架说明：modules/order/dto/create-order.dto.ts —— 占位骨架，开发阶段实现
 *
