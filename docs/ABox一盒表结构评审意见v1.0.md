...
>
> **2026-09-15 增补（M2 实施期发现 P0-7）**：`ab_withdraw` **提现申请单缺失** ——
> 余额流水（`ab_balance_log`）只记发生额，无法承载提现单的**审批状态机**（待审批 → 已批准 →
> 打款中 → 已到账 / 已驳回 / 打款失败）、审批人与驳回原因、以及灵活用工出款批次号；
> 而接口 L12 / L13 与后台 D45 / D46 均以它为载体。已随 M2 一并补齐（DDL 见 ER v2.1 §3.5），
> **评审后表数 24 → 25 张**，与 `ALL_ENTITIES` 逐张对齐（由门禁基线清单校验）。
...
开发推进到 M2（团长端）时，发现两处**文档与实现的口径冲突**，经裁定如下 —— 二者都直接决定 `/leader/apply` 的写法，故先行冻结：

| # | 议题 | 冲突原状 | 裁定 | 落点 |
| --- | --- | --- | --- | --- |
| **①** | 团长在职状态判据 | 《接口规范》§1.5 / §4.5 / §4.7 写 `status='active'`（字符串），实体实为 `tinyint`（`1` 在职 / `2` 停职） | **以实体为准**：`status = 1` 判定在职 | 代码本已按 `leader.status === 1` 实现（`auth.service`、`order.service`）；**本次仅回改《接口规范》三处措辞**，无代码变更 |
| **②** | `floor`（楼层）字段 | 《接口规范》U1 / U3 / L17 以 `floor` 为契约字段，但 ER v2.1 已删除且 `ab_team_leader` 无该列 —— 而 `ValidationPipe` 开了 `forbidNonWhitelisted`，前端按契约传 `floor` 会**直接 10001**（非静默忽略） | **在 `ab_team_leader` 补回 `floor`**（`VARCHAR(32) DEFAULT NULL`） | ER §4.2 恢复该列 + `leader.entity.ts` + init 迁移 + 种子同步；`ab_user.floor` 保持删除不变 |

**裁定理由**

- ①：24 张表中其余状态位**统一为 tinyint**（`1` 正常 / 在职），单独为团长用字符串会造成双写与歧义；且已实现代码即为 tinyint 口径，改文档的成本与风险最小。
- ②：楼层是**同楼内按层分群分发、按层聚合取餐提醒**的核心组织维度（12 楼 / 5 楼群 / 14 团长的组织粒度），接口契约一直在用；若按 ER 删除则必须回改三处接口契约并留下「有参数不落库」的死字段，属回退。

> 以上两项已同步回写《接口规范 v1.0》与《ER v2.1》，代码无需返工。
