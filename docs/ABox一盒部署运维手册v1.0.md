...
> 配套脚本：`scripts/deploy.sh` · `rollback.sh` · `backup-db.sh` · `restore-db.sh` · `healthcheck.sh` · **`ops-daily.sh`**（日巡检 · M5-12）· **`install-cron.sh`**（cron 幂等安装 · M5-12，声明唯一真相 `deploy/cron/abox-ops.cron`）
...
> ⭐ **M5-12 起有页面了**：运营后台 → 系统 → **跑批时刻表**（`/system/schedule`，D64/D65），
> 本表 + **实际注册的 cron** + 目标日语义 + 一键补跑都在这页上。
> ⚠️ 值班时**优先看页面而不是看这张表**：本表是**出厂口径**，而运营改过配置后
> 真正生效的时刻是运行时由配置派生的（页面上「生效时刻」与「实际注册」两列）。
...
### 7.6 服务器周期性运维动作（cron · M5-12）

> 在此之前，「备份」与「巡检」两件事**只写在文档里、没有任何一处会真的执行它们** ——
> 而这两件事的共同特征是：**不做也不会立刻出事**。所以它们的缺失不会在任何日志里出现。

**声明唯一真相**：`deploy/cron/abox-ops.cron`。安装器只做「读模板 → 替换占位符 → 幂等写入」。

```bash
./scripts/install-cron.sh --dry-run     # 先预览渲染结果（**首次必须做这一步**）
./scripts/install-cron.sh               # 安装 / 更新（幂等：先删旧块再写新块）
./scripts/install-cron.sh --list        # 查看当前安装了什么
./scripts/install-cron.sh --uninstall   # 摘掉本项目块（用户自己的 crontab 条目不动）
```

| 时刻 | 动作 | 说明 |
| --- | --- | --- |
| 每日 **05:00** | `scripts/backup-db.sh` | 8 个跑批任务最晚 04:00（对账）跑完，05:00 落备份不会备到「跑了一半」的账 |
| 每日 **08:30** | `scripts/ops-daily.sh` | 人上班前出结论（见下） |

**为什么用脚本而不是直接往 crontab 里写命令**：

- `install-cron.sh` 是**标记块替换**而不是追加。追加式安装器跑三次就有三份备份任务，
  而**重复的 cron 不会报错**，只表现为「磁盘莫名涨得快」（备份文件还互相覆盖）。
- 它**拒绝**写入任何还残留 `{{占位符}}` 的内容：crontab 会**原样接受**一条语法错误的命令行，
  然后每天定时执行它 —— 表现为「cron 什么都没干」，而 `crontab -l` 看上去完全正常。
- 它**不自己生成任何 crontab 行**（内容全部来自模板），否则模板与安装器就成了两份表述。

**`ops-daily.sh` 检查三件事，收成一个退出码**：

```
退出码：0 = 通过（允许有 WARN）· 1 = 有 FAIL · 2 = 用法 / 环境错误
```

| # | 检查 | 为什么它在「探针绿」之外还必要 |
| --- | --- | --- |
| ① | 探针**两级**（liveness + readiness） | 进程活着 ≠ 依赖通、消费者齐；两者处置动作不同（重启 vs 摘流量） |
| ② | **备份新鲜度**（判**产物的年龄**，不判备份脚本有没有报错） | cron 只看脚本退出码；「文件其实没写出来」在退出码上看不出来。另单独拦**空导出**（文件在、时间新、大小 0 —— 到恢复那天才知道是空的） |
| ③ | **磁盘余量**（判**可用百分比**） | 磁盘满的第一个症状通常不是宕机，而是 MySQL 写入偶发失败 —— 最容易查错方向 |

```bash
./scripts/ops-daily.sh                                   # 完整日巡检
./scripts/ops-daily.sh --url http://127.0.0.1:8080/api/v1 # 从网关侧探测
./scripts/ops-daily.sh --skip-backup                     # 当天备份尚未接入时（结论会如实标注「不覆盖备份」）
```

⚠️ **`ops-daily.sh` 会显式清掉 `http_proxy` / `https_proxy` / `ALL_PROXY` 后再探测。**
这不是洁癖：`curl` 默认尊重这些变量，于是「探测 `127.0.0.1:8080/health`」会变成
「探测代理主机」，而**代理对连不上的上游回 `502 upstream connect failed`，与本机服务真的挂了长得一模一样**。
（本项目实测踩到过：开发机上已设 `http_proxy`，探针打一个**确实在监听**的本地端口却拿到代理的 502 ——
若走进巡检，它会每天报「服务不可用」而服务一直是好的，直接把人训练成忽略告警。）

⚠️ **从未在真实服务器上执行过**（开发机无 Docker / 无 `.env.prod`）：只做过渲染验证、
`bash -n` 静态检查，以及**两条分支的实测**（探针 200 → 退出 0；探针不可达 → 退出 1；
新鲜备份 → OK；1 字节备份 → FAIL）。首次上服务器请先 `--dry-run`、再手工跑一次
`ops-daily.sh` 核对输出，**最后**才交给 cron —— 与 `deploy.sh` 同一纪律。

⚠️ **时区**：cron 用宿主机本地时区，业务口径是 `Asia/Shanghai`。这里**刻意不设 `CRON_TZ`** ——
设了就成了第三个时区来源（宿主机 / 容器 `TZ` / `CRON_TZ`），而「跑批按时区错一小时」
在本项目里是**不报错**的（见 §六 禁发窗口）。正确做法是把宿主机（或 compose 的 `TZ`）固定为 `Asia/Shanghai`。
...
- [ ] `./scripts/ops-daily.sh` → 退出码 **0**（它把探针两级 + 备份新鲜度 + 磁盘余量收成一个数；
      cron 已按 §7.6 在 08:30 自动跑一次，此处是人工复核那一份的结论）
- [ ] `./scripts/healthcheck.sh` → 通过
- [ ] `GET /admin/queue` → `durable:true`、无增长中的 failed
- [ ] 查 `ab_operation_log`：有无 `对账不平` / `任务重试耗尽`
- [ ] 跑批是否按时执行（对照 §7.2 时刻表；**优先看后台「跑批时刻表」页**）
- [ ] 当日报错数、订单数是否符合预期
...
| R2 | **生产库从未执行过迁移**（开发全程用 sqlite 的 `synchronize` 自动建表）。⭐ **M5-2 升级**：**「迁移推演 ≡ 实体结构」已由 `schema:parity` 门禁机械保证**（全库 **28 表 / 407 列**逐表逐列对账，且检查器**每次运行自证能报出人为缺口**）；⚠️ **但「结构对」≠「跑得通」** —— 本机无 Docker / 无 MySQL，**MySQL 上真跑一次仍未做过** | 首次迁移可能因 **MySQL 方言差异**（如 `information_schema` 守卫行为、`JSON` 列、字符集 / `ENGINE` 子句）失败；⚠️ 另注：迁移现有 **4 支**（`1700000000000-init` / `1700000000001-parity-fix` / `1700000000002-index-commission-meal` / `1700000000003-dish-rating`），**四支都要 applied** 才等于实体结构；⚠️ **漏掉后两支的后果**：`ab_dish_rating` 表不创建 ⇒ 口味评价（U20 逐菜三键 / D69 红黑榜）**上线即 500**，且 `ab_commission` / `ab_meal` 相关索引缺失 | 首次部署前在**空库**上预演一次；`typeorm migration:show` 必须列出 **四支** 且都为 `[X]`（`Init1700000000000` / `ParityFix1700000000001` / `IndexCommissionMeal1700000000002` / `DishRating1700000000003`）；再跑一次 `gate.mjs schema:parity` 确认结构一致 |
...
| R10 | ⭐ **一期只允许「一个 API 实例」承载** —— `currentTimeline()`（生效业务时刻）是**进程级单例**，配置变更只在**当前实例**内热重载 | 多实例下会出现「A 实例已按新截单时刻拒单、B 实例还按旧时刻放行」：**两边都不报错**，表现为「同一分钟下单，一台过一台不过」——比配置没生效更难查（配置页显示已生效） | **本期不要横向扩容 API**。若必须多实例：先改成「配置变更经 Redis 发布订阅广播」（`order-timeline.ts` 已预留 `onTimelineChange()` 订阅点，接上即可），**并写一条黑盒断言**证明第二个实例也刷新了（`GET /admin/schedule` 的 `effectiveAt` 在两实例上一致）—— 在此之前多实例属于**未验证形态** |
| R11 | **运维脚本与 cron 从未真机执行**（同 R1）：`ops-daily.sh` / `install-cron.sh` / `deploy/cron/abox-ops.cron` | 巡检可能因宿主差异（`df` / `find` / `curl` 版本、权限）行为不同 | 按 §7.6：先 `--dry-run` → 再手工跑一次 `ops-daily.sh` 核对输出 → 才交给 cron |
...
| 2026-09-17 | v1.0.1 | **M5-2 同步：R2 升级** —— 「迁移推演 ≡ 实体结构」已由 `schema:parity` 门禁机械保证（**当时**为 27 表 / 394 列；⚠️ 该口径已于 **v1.0.3** 更正为 28 表 / 407 列）；⚠️ 但**不写成已验**（本机无 Docker/MySQL，MySQL 真跑仍待空库预演）；R2 的「上线前必须做什么」补明 **（当时的）两支迁移都要 applied** + 用 `migration:show` 核对 + 复跑 `schema:parity`（⚠️ 现为**四支**，见 v1.0.3） | 项目总监 |
| 2026-09-18 | v1.0.2 | **M5-12 同步：运维日巡检进 cron + 多实例前提如实标注** —— 新增 §7.6（`deploy/cron/abox-ops.cron` 声明唯一真相 + `install-cron.sh` 幂等安装器 + `ops-daily.sh` 三检查收成一个退出码）；§7.2 补「后台已有跑批时刻表页 `/system/schedule`」；§十 日巡检首项改为 `ops-daily.sh` 退出码；新增遗留风险 **R10（一期只允许单 API 实例 —— `currentTimeline()` 是进程级单例，多实例会造出「两台各自生效、两边都不报错」）** 与 **R11（运维脚本与 cron 从未真机执行）**；⚠️ 一并记录 `ops-daily.sh` 实测踩到并修掉的**代理陷阱**（探针会经 `http_proxy` 打到代理上、拿回与「服务挂了」同形的 502） | 项目总监 |
| 2026-09-25 | v1.0.3 | ⭐ **R2 口径更正（迁移支数 / 表列数）** —— 原文写「**2 支迁移**」「**27 表 / 394 列**」，实测为 **4 支迁移 / 28 表 / 407 列**（`gate.mjs schema:parity` 2026-09-25 实跑）；⚠️ **照原文只核对两支，会漏掉 `1700000000002-index-commission-meal` 与 `1700000000003-dish-rating`**，后果是 `ab_dish_rating` 表在生产**根本不创建** ⇒ 口味评价（U20 逐菜三键 / D69 红黑榜）**上线即 500**，且 `ab_commission` / `ab_meal` 相关索引缺失。「上线前必须做什么」已同步改为「`migration:show` 必须列出**四支**且都为 `[X]`」 | 项目总监 |
