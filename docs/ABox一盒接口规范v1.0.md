...
| 最近修订（本轮 · 未实装端点标注） | 2026-09-20 · **契约「未实装」口径落成（纯文档 · 零代码改动 · 零新增端点 · 零 DDL）**：外部测试进场前，把「规范里有行、代码里没有」的缺口逐条落成**明示契约**。① ⭐⭐ **新增 §1.8「本期未实装端点清单（对外测试必读）」** —— 14 条逐条给「性质 + 为什么不实装 + 何时补」；其中 **U12 是刻意不设端点（由 A2 承担）、不是缺口**，其余 13 条为一期未实装。② ⭐⭐ **判据可复算且经交叉验证**：仓库内既有门禁 `route:audit`（**NestJS 运行时反射**）实测 146 端点；另在工作区根新增 `tests/tools/audit-contract.mjs`（**静态**解析 controller 装饰器 × 本文件端点表）做对账 —— 两法端点集合**完全相等**（146 = 146 · 交集 146 · 双向差额 **0**），**两个独立取数方法一致**才算数。③ ⭐ **豁免会自收紧**：对账工具认「行内显式声明」为已声明（**不静默忽略** —— 静默忽略比漏报更快把人训练成忽略告警），把声明删掉即重新变红。④ ⭐ §五 节首加一次性提示（10 条未实装讲清，省得测试人逐条踩）。⑤ ⚠️ **顺手更正一处自相矛盾**：§1.6「图片」行原写上传走后 `POST /me/upload-token`，而 §3.1 **U4 定义为 `GET`** —— 同一端点两种方法、两处都不报错，已更正为 U4 `GET` 并标注未实装。⑥ ⚠️ **不改任何代码**：本轮只把**既有事实**写进契约，不新增/不修改端点，`gate.mjs` 22/22 不受影响。 |
...
| 最近修订（M5-19） | 2026-09-21 · **第三方测试报告修复 · 工程本批（零 DDL · 零新增错误码 · 门禁仍 22 道 · 表数仍 27）**：① ⭐⭐ **PR-01 自助取消（U11）两层补齐幂等** —— 控制器补 `@Idempotent({ scope: 'order-cancel', required: false })` + `@UseInterceptors(IdempotentInterceptor)`（**刻意非必填**：提为必填是**破坏性契约变更**，而真正的并发防护在服务层）；服务层把「事务外**快照读** → 事务内**无条件** `update`」改为**原子占位** —— `UPDATE ab_order SET status='cancelled', cancelled_at=?, version=version+1 WHERE id=:id AND status=:from`，以 `affected = 0` 判归属并抛 **`30003`**（**fail-closed**：事务整体回滚、**钱一动不动**；静默返回会给出「取消成功、退款已发起」的**假回执**）；余额动作（`releaseBalance` / `refundBalance` / `buildRefundRow`）**移到占位成功之后**。⚠️ 幂等键**只拦「同一键」的重放**（`10006`），拦不住「两个不同键」或「一端带键一端不带」的并发 ⇒ **两层覆盖不同情况，缺一有缝**；⚠️ 串行重放**本来就测不出来也不需要测**（第二次会因已 `cancelled` 返回 `30003`，**恰好掩盖问题**），唯一窗口是「两个事务同时读到 `pending_pay`/`paid`」⇒ **真机 MySQL 并发验证属待办**。② ⭐⭐ **余额写入收口到唯一口 `updateBalanceRow()`** —— `freeze` / `release` / `consume` / `refundBalance` 四处原先写作 `version` **只进 SET、不进 WHERE** ⇒ **乐观锁是装饰性的**：读-写之间若有另一路写点（T+1 佣金入账 / 另一笔退款 / 双端取消）提交，这里会把**过期值**写回去（丢更新），且 `ab_balance_log.balanceAfter` 记的是**自己算的那个错值** —— **流水与余额一起错、彼此自洽**，事后对账抓不住；现 `WHERE id = :id AND version = :v` + `affected = 0` 抛 **`40018 BALANCE_CONCURRENT_MODIFIED`**（该码 M5-7 即为此定义、此前在本模块**不可达**）。③ ⭐⭐ **送达时刻收敛到真源** —— ⚠️ **先纠正一处口径误判**：`set_meal.delivery_arrival_time` 在 D58 白名单里是 `wiring = 'live'`（`isEditable() === true`）⇒ **后台一键可改**，此前「一期不可配」的说法**不成立**；真源 = `common/utils/order-timeline.ts` 的 `DEFAULT_TIMELINE.arrival`。逐处收口（**运行时 11 处 + 潜伏 2 处**）：`team-leader/workbench.service.ts` ⭐ **同 payload 双真相**（`expectAt` 手写 `'11:30'` 紧邻 `expectAtIso` 派生 ⇒ 改一次配置**当场分裂**）· `team-leader/share.service.ts` 分享语（**离线转发，改后无法纠正**）· D66 `deliveriesLate` 的 `hint`（与 `isPastArrival` 同源）· `admin/config/config.specs.ts` 与 `admin/template/message-template.specs.ts` **删掉重复陈述**（前者文件头本就禁止「在别处再写一遍时刻」，属**自我违反**）· `order/order-state-machine.ts` 潜伏死文案（`buildTimeline` 的 `userStatusText(n) \|\| step.pendingText` 被 `ORDER_STATUS_VIEW` 11 态**全覆盖短路**）· 端上 `constants/index.ts` 的 `TIME_ANCHOR` **整表删除**（**10/10 成员全仓零消费**，是时间轴的第二份表述）。④ ⭐⭐ **新增 3 个下发字段**（端上取不到真源，只能由服务端派生下发）：**U1 `deliverAt`** · **U3 `deliverAt`** · **U10 `pickup.expectAt`**，三者均 = `formatTimeOfDay(currentTimeline().arrival)`，与同响应里既有的 `cutoffAt` / `expectAtIso` **同源**；端上 `order-create.vue` / `order-detail.vue`（取餐卡 + 「配送途中」提示）/ `leader-invite.vue` 三页改为**只展示响应值**。⑤ ⚠️ **有意保留 1 处并双标注** —— 用户协议正文（`apps/miniprogram/src/constants/agreements.ts`）的 `11:30`：协议页**刻意零网络请求**（离线可读、逐字可审，文件头第 3 条已裁定为「人工镜像」），**不能**改为从响应取；已在文件头与正文字符串处**双标注**「人工同步点」并**点名真源**（彻底消除需改合规文本，属**待裁定**）。⑥ ⚠️ **顺带纠正 2 处文档↔代码漂移** —— U10 示例里 `delivered` 节点的 `text` 原写 `预计 11:30 送达`（那是 `pendingText`），实装因 `\|\|` 短路**恒为 `待取餐`**；`pickup` 示例补 `expectAt`。⑦ 验收：`gate.mjs all verify` **22/22**（`format` 4 文件按仓库格式落盘 · `security:scan` 因三个一次性探针含硬编码口令而**移交工作区根 `tests/`** 后转绿）；e2e 断言数**不变**（m1 64 / m2 128 / m3 1055）。详见《里程碑计划》§五「子批次 M5-19」 |
| 最近修订（M5-20） | 2026-09-23 · **账号注销 U19（提审阻塞项收口）· 工程本批（零 DDL · 零新表 · 表数仍 27）**：① ⭐⭐ **补上提审硬要求** —— 《提审自检清单 v1.2》第 10 条要求小程序提供**账号注销入口**，此前系统里**一行相关代码都没有**（只在合规清单 §七 row 2 挂着「未实装」），与 U16 协议阅读页并列属**提审阻塞对**；本批把它落成**可点的原生页**（`pages/account-cancel/account-cancel`），入口在「我的 → 设置 → 账号注销」。② ⭐⭐ **「注销」= 匿名化 + 停用，不是删行**：`ab_user.status = 3`（新增 `UserStatus.CANCELED`） + `deleted_at` + `nickname = '已注销用户'`，并清空 `avatar_url` / `phone` / `phone_hash` / `building_id` / `team_leader_id` / `subscribe_flag`（共 7 个字段，由**服务端下发** `clearedFields`）。⭐ **`openid` / `unionid` 刻意保留** —— 删掉它只会让**同一个人**下次登录又注册成一个全新账号（同一微信两套账，比留一行匿名数据更糟）；「注销后不能再登录」由 `status = 3` 保证，不靠删标识。③ ⭐⭐ **再登录 → 新码 `20014`**（`AuthService.login()` 查库后**先于黑名单 `20006`** 判 `CANCELED`）：**刻意不复用 `20006`** —— 黑名单是**平台的决定**、注销是**用户自己的决定**，混成一个码客服就无法从错误码判断该走「解封」还是「恢复账号」。（与 `40015`/`40002`、`40017`/`40013` 的取舍同源：形态相似、**排查入口不同**。）④ ⭐⭐ **三道闸门 fail-closed → 新码 `20015`**：在职团长 / 余额或冻结 > 0 / 有未终态订单；`data.reasons` 下发**原因码数组**（`leader`/`balance`/`orders`）+ `data.pendingOrders`。⭐ **三道全查一遍再报**（逐个报会让用户「改一条、再被拦一条」来回试）；⭐ 为什么不「连余额一起清掉」：注销不可逆而余额是**钱**，一期没有「余额清零 / 退款到微信」的自动通道，自动清掉 = **用户的钱被系统吞掉且不会有任何报错**；fail-closed 的正确含义是把他挡在**一次失败**上。⑤ ⭐⭐ **二次确认词由服务端裁定**（`confirmText` 须逐字等于「注销账号」，否则 `10001`）：端上自绘的确认弹窗**不构成证据**，服务端认这个词才使得**任何调用方**都必须走同一道确认。⑥ ⭐ **原子占位**（本批自查发现并当场修掉）—— 初版写成「查库判 status → 再无条件下 UPDATE」，即 M5-19 刚立过的「**读-再写**」形状：两个并发请求同时读到 `status = 1` 会**双双注销成功**，第二个人拿到的是「刚注销成功」的**假回执**。改为 `WHERE id = :id AND status != 3` + `affected = 0` 判归属 → `20014`；查库那次降级为**快路径**（错误码更准、少一次写），**不构成并发防护**。⑦ ⭐ **不碰资金账**：注销**不删** `ab_commission` / `ab_balance_log` / `ab_order` 行（财务凭证留存先于「删除个人数据」），注销是**身份层**动作。⑧ ⭐ **口径由服务端下发**（`clearedFields` + `note`），端上不复制第二份文案；`note` **必须**写清「与订单、退款、佣金相关的记录按法律法规要求保留」—— 否则用户以为「注销 = 数据全没了」，而客服在后台仍看得到他的历史订单，两边说法不一致就是**虚假告知**。⑨ ⭐ 端上：成功后**就地切「已注销」终态屏且不提供返回**（返回栈会把用户带回一个已失效的「我的」页）；`request.ts` 对 `20014` **清本地登录态但不重试**（重试会变成「登录 → 被拒 → 跳登录页 → 再登录」的**死循环**）；协议文本 §五 注销条款由「联系客服」改为**自助路径**（原文与实现不符即合规风险）。⑩ ⚠️ **已知边界（写成断言而非遗漏）**：已签发的旧 token 在过期前仍可用，但读到的是**已匿名化**的资料 —— 与 `20006` 黑名单**同口径**（状态只在**登录时**判，不做每请求查库；后者要给全站加一次 DB 往返，属**待裁定**项）。⑪ 落点：实装原**空壳** `modules/user`（`UserService` 此前只有骨架）+ 新文件 `common/constants/user-status.ts` · `modules/user/dto/user-cancel.dto.ts` · 端上 `pages/account-cancel/`；`pages.json` / `mine.vue` / `api/user.ts` / `api/request.ts` / `constants/agreements.ts` / `agreement.vue` 同步。⑫ **e2e `§9` 新增 24 条断言**（m1 **64 → 88**，逐条实跑）：二次确认词被拒后**账号未被误注销** · 两道闸门**互不误报**（干净账号 `reasons` 恰为 `["orders"]`）· 取消在途单后闸门**随即放开**（证明拦的是「在途」而非「下过单」）· `clearedFields` 恰为 7 项 · 落库三查（`status=3` + 软删 / 资料已匿名化 / **`openid` 仍在**）· 订单行未被删除 · 再登录 `20014`（且 ≠ `20006`）· 重复注销 `20014` · 旧 token 读到匿名化资料。⚠️ 样本用 `dev:qa-cancel-<时间戳>` **现造干净账号** —— 种子里的 5 个用户**本身全是团长**，拿他们做「成功注销」样本会**必然**撞上闸门①，永远测不到成功路径。详见《里程碑计划》§五「子批次 M5-20」 |
| 最近修订（U2 · 口味评价与菜品迭代闭环） | 2026-09-25 · **P1-U2 口味评价（U20 提交 + D69 红黑榜 · 新增第 28 张表 `ab_dish_rating` · 第 4 支迁移 · 新增错误码 30020·30021）**：真源《ABox一盒创业系统地图与开发路线 v1.0》U2 —— P1 验收判据「**能说出本周最差的三道菜**」。① ⭐⭐ **§三 新增 `U20 POST /orders/{orderNo}/rating`（逐菜三键 · 一次提交定稿）+ §6.6 新增 `D69 GET /admin/stats/dish-rating`（口味红黑榜）** + 新增「U2 实现口径」表（28 项）；② ⭐ **四项开工前裁决**：粒度 = **逐菜三键**（「一键」体现在一屏 4 行点完一次提交，不是整餐一个分 —— 按菜聚合的前提）；红黑榜落点 = **P35 菜品热度页第二个 Tab**（同域互补 · 零导航变更 · F5 先例）；红线阈值 = **代码常量**（避开 D-1 `fallback` 恒不生效缺陷，出参仍下发 `redLineThreshold` 端上不写死）；**一次提交即定稿不可改**（→ `30021`，**刻意不复用 `10006`** —— 重放返回首次成功结果，与「拒绝新提交」语义相反）；③ ⭐⭐ **`badCount` 与 `monthBadCount` 分道**：区间投诉 vs **锚点月**投诉 —— 月初若按区间算「本月投诉」会把上月旧账计入、红线天天误报（e2e §40 用「2025-12-31 差评进 30d 窗口、不进 2026-01 锚点月」钉死；单测同款）；④ ⭐ **区间切 `meal_date`（吃到的日子）**而非评价提交日 ——「本周最差的菜」对齐「本周吃到的菜」；`resolveStatsRange` 从 StatsService 私有方法**上提为纯函数**（D49/D69 共用，杜绝「缺省锚=今日」两处各自演化）；⑤ ⭐ **U10 详情补 `dishes[].dishId` + 内嵌 `rating` 块**（`canRate/rated/ratedAt/items`）—— 评价卡的菜键与状态**唯一来源**是详情出参（端上不按 status 自推可评性）；⑥ ⭐ **聚合收敛为纯函数** `aggregateDishRatings()`（零 IO · 月界/排序/截断可单测 —— `dish-rating-agg.spec.ts` 6 条；F5 教训：导出写「供单测」就必须真有人测）+ 快照纪律（`dish_name`/`supplier_name`/`slot` 落库定格，改名不改史）；⑦ **e2e**：m1 新增 U20 用户流 **16 条**（88 → 104 · 状态闸门 / 越权 / 菜属 / 一次定稿 / 终态回显 / 快照落库 / **节末夹具还原** —— 不还原会让后续套件的开篇 U6 夹具红在 30004）；m3 新增 **§40 · 18 条**（固定锚 2026-01-10 + 手算期望值对账 · 空态/月界/黑榜序/权限/还原，1092 → 1110）；⑧ 表数 **27 → 28**（`uk_dish_rating_order_dish` 唯一键兜底「一次定稿」）· 迁移 **三支 → 四支**；jest **6 → 7 套 / 45 → 51 条**。 |
| 最近修订（F5 · 通知触达矩阵） | 2026-09-25 · **F5 通知触达矩阵补强（编排 + 到达率 + 截单提醒场景 · 零 DDL · 零新增错误码 · 表数仍 27）**：真源《ABox一盒创业系统地图与开发路线 v1.0》F5 —— 原文「已有 `message` 模块，缺的是**按人按场景编排 + 到达率回看**」，P1 验收判据「**截单与取餐提醒的到达率可见**」。① ⭐⭐ **§6.7 新增 `D67 GET /admin/messages/reach`（到达率回看）+ `D68 POST /admin/messages/orchestrate`（按人按场景批量触达）** + 新增「F5 实现口径」表（23 项）；② ⭐ **M3-12 场景清单 6 → 7**：新增第 7 行 `cutoff_remind`「截单提醒」（`requestSubscribe=true` · `wiring='live'` · 一期 `enabled=0`），**追加在数组末尾**以保住「前 5 行 = 原型」的可核对性（该结构被 e2e §24 锚定）；`NOTIFY_PAGES` 新增 `orderCreate`（促单要落到**能下单的页**）；③ ⭐⭐ **安全性靠「结构」而非「自觉」**：受众入参**只接受预定义选择器**（不收 `userId[]` —— 收一个任意列表就等于开了**群发后门**），且选择器与场景**强绑定**（串用 → `10001`）；`dryRun` **缺省 `true`**（默认只算不发，要真发须显式 `false`），默认值**定义在服务层**、DTO 刻意不给默认值（否则「未传」与「传 `false`」在契约上不可分）；④ ⭐⭐ **`ab_message` 没有 `scene` 列** ⇒ 场景藏在 `payload.scene`，而 `JSON_EXTRACT` 属**驱动相关语法** ⇒ 按场景聚合**必须在服务端内存完成**（同 D44 发票先例）；⭐ **范围过滤走 `created_at`（非空列）、日归属优先 `sent_at`**，两者不可混用 —— 用 `sent_at` 过滤会把「已尝试未成功」整批漏掉，而那正是到达率最该被看见的部分；⑤ ⭐⭐ **两个「不许混淆」的显示口径**：`reachRate` 分母是**已尝试投递数**（不是受众数）；**无记录时是 `null` 而非 `0`**（「没发过」与「发了全失败」是两回事，显示 0% 会让运营去查一个不存在的故障），端上渲染为 `—`；无记录时 `note` **明写原因**（一期无微信账号 → 模板 ID 全空 → 闸门拦住 → 跳过且不写日志）；⑥ ⭐⭐ **一期必然「一条也发不出去」，且如实呈现**：无微信账号 ⇒ 启用闸门 fail-closed 拦住所有订阅消息场景 ⇒ `notify()` 在第 2 步返回 `delivered=false`；**这不是缺陷，是当前真实状态** —— 出参照样给真实 `audienceSize`，让运营看得见「今天若已开通，会有多少人收到」，**不许粉饰成「已发送」**；⑦ ⭐ **`scenes[]` 含 0 投递的场景**（全部 7 个，否则一期整页空白）+ `orphanScenes[]`（已下线场景的历史记录，**不静默丢**）；区间缺省近 7 天、上限 **90 天 = `ab_message` 保留期**，非法日期 / `from > to` / 超限一律 `10001`（**不静默回落**）；⑧ ⭐ **第 27 张表零改动**（场景定义在代码里，`ab_message_template` 只存可编辑部分）；**错误码零新增**（场景/选择器/串用/日期/区间全走 `10001`，全部 fail-closed）；⑨ ⚠️ **未做的部分如实登记**：**企业微信 / 短信**两条通道一期不做（2026-09-25 裁决）· **自动跑批**未接入（真源 F5 依赖「订阅消息模板审核」，一周触达入口是编排端点，接跑批时直接调同一服务方法、业务逻辑一行不改）；⑩ ⭐ **顺手校正一处文档漂移**：§6.7 的「实现状态」行原写「`D59–D60` 待后续批次」，与**本文档下方 M3-12 小节**及实装（`admin.controller.ts` 的 `@Get('templates')` / `@Put('templates/:id')`）**自相矛盾**（M3-12 落地时未回改），已更正并留痕；⑪ 验收 `scripts/e2e-m3.mjs` **§39 · 26 条断言**（场景接线 + 安全默认 + 入参闸门 + 真发 + 到达率 + 权限 + 夹具还原，**不依赖下单窗口**），m3 **1066 → 1092**；`§24` 同步 4 处（6 → 7 / `live` 3 → 4 / 描述去硬数字）。 |
...
| **图片** | 返回 COS 完整 URL；上传走后端签发的临时密钥（**U4** `GET /api/v1/me/upload-token` —— ⚠️ 原文误写 `POST`，与 §3.1 U4 定义**自相矛盾**，本批更正；**一期未实装**，详见 §1.8） |
...
### 1.8 本期未实装端点清单（对外测试必读）

> ⚠️ **为什么单列一节**：下列端点**在本文件里有行、在代码里没有**。这类缺口**不会让任何门禁变红**
> （`eslint` / `tsc` / `build` / `e2e` 全绿），但外部测试人若**照本表逐条验证**，会把它们**整批记为 bug** ——
> 第一周全在报无效问题。本节就是为此而设：**要么补实装，要么在这里明说未实装**。
>
> **复核方法（可复算 · 两个独立取数方法交叉验证）**：
> ① 代码侧：仓库内既有门禁 `route:audit`（`apps/api-server/src/common/security/route-audit.ts`，**NestJS 运行时反射**）
>    —— 实测 **34 个 `*.controller.ts` → 146 个端点**；
> ② 对账侧：工作区根 `tests/tools/audit-contract.mjs`（**静态**解析 controller 装饰器 × 本文件端点表），
>    两法端点集合**完全相等**（146 = 146 · 交集 146 · 双向差额 0）⇒ 下方结论可采信。
>    ⚠️ 该工具在**工作区根**、不入仓（见《本地开发手册》§6.1 沙箱替代跑法）。
>    ⚠️ **未声明缺口 > 0 时它非零退出**（`--report-only` 可只报告）—— 判据是「行内有没有写明为什么」，
>    没写就算缺口。**豁免不是静默忽略**：把声明删掉，它立刻重新变红。
>
> **本表 = 2026-09-20 复核结果 · 共 14 条**（13 条一期未实装 + 1 条刻意不设端点）。
> ⚠️ 若某条**已被实装**（本表过期），请改**脚本口径**而非手改数字 —— 手改一次即失去可复算性。

| # | 端点 | 性质 | 为什么不实装 / 何时补 |
| --- | --- | --- | --- |
| **U4** | `GET /me/upload-token` | 一期未实装 | 无消费点（端上与后台**全仓 0 引用**）⇒ 不存在「入口点不动」；缺的是 **COS 临时密钥签发**（需存储桶与密钥）⇒ **随云资源到位 + 「我的」页头像功能一并补** |
| **U12** | `GET /me` | ⛔ **刻意不设端点** | 用户信息**由 A2 `GET /auth/me` 承担**（M5-10 裁定：同一份数据两个端点必然漂移）。**不是缺口** —— 照表验会拿到 404，那是**预期** |
| **U15** | `POST /me/subscribe` | 一期未实装 | 上报需一张「谁授权了哪个模板」的表（**新增 DDL**）；一期**无微信账号 ⇒ 无模板 ID 可授权 ⇒ 无法验证**。只做读侧（U18） |
| **U16** | `GET /me/agreements` | **本端点不设**（改由端上内置原生页承担）· ✅ M5-18 收口 | 协议正文是**静态文本**，一期**不设端点**：端上 `pages/agreement/agreement?type=user\|privacy` 直接渲染 `@/constants/agreements`（正文唯一真源），**零网络请求、零新表**。**不走 web-view** 的理由：web-view 需「业务域名白名单 + ICP 备案 + HTTPS + 域名根目录校验文件」，会把**提审**绑到 M5 的外部阻塞项上，且审核期域名不通**即不通过**。提审要求（《提审自检清单 v1.2》第 11 条）由该页面满足；⚠️ 仍须**法务定稿**（当前 `version=1.0-draft`，页面显示草案条） |
| **S4 / S5 / S6** | `GET`·`POST /supplier/dishes` · `PUT /supplier/dishes/{id}` | 一期未实装 | 供应商一期**无独立系统**（共用后台 `:5173`）；菜品是**平台端主数据**（运营走 `/admin/dishes`）。端上 P23 页已改**如实说明页** |
| **S7** | `POST /supplier/dish-applications` | 一期未实装 | 一期无「供应商报名参加某日套餐」流程；套餐编排由运营在后台完成（D1–D7）。端上 P24 页为如实说明页 |
| **S8** | `GET /supplier/history` | 一期未实装 · 无入口 | 供应商导航 `SUPPLIER_NAV` 未挂该页；一期「历史」由 **S9 结算明细按日**承担 |
| **S10** | `GET /supplier/settle-account` | 一期未实装 · 无入口 | 对公账户由**运营在后台维护**（D28）；供应商侧结算入口只有 S9 |
| **S11** | `POST /supplier/invoices` | 一期未实装 · 无入口 | 一期开票台账在**运营侧** D44（`/admin/finance/invoices` · 派生视图 · 零 DDL）；供应商侧申请入口未做 |
| **S12 / S13** | `GET`·`PUT /supplier/profile` | 一期未实装 | 资质由**运营在后台审核维护**（D26）。端上 P26 页为如实说明页 |
| **S14** | `GET /supplier/agreement` | 一期未实装 · 无入口 | 协议正文一期未落地 |

> **给测试人的一句话**：上表 14 条**一律不要记为 bug**（U12 除外项也不报 —— 它是**刻意没有这个端点**）；
> 其余端点照本文件逐条验即可。判据来源是脚本，**过期改脚本、不改数字**。
...
| # | 方法 | 路径 | 说明 | 关键出参 |
| --- | --- | --- | --- | --- |
| U1 | GET | `/home/daily` | 明日套餐 | `{ mealDate, publishAt, cutoffAt, deliverAt, canOrder, countdownSec, dishes:[{name,image,category,supplierName,supplierId}], rice, priceFen:2580, leader:{id,name,building,floor,source} }` |
| U2 | GET | `/home/history` | 历史套餐归档（M01-02） | 分页 `{mealDate, dishes[], priceFen, ordersCount}` · ⚠️ **端上首页已不展示**（M5-17），接口保留 |
| U3 | GET | `/leader/invite/{leaderCode}` | 团长邀请落地（M01-03） | `{ leaderName, building, floor, slogan, deliverAt }`（**免登录**） |
| U4 | GET | `/me/upload-token` | COS 临时上传密钥 · ⚠️ **一期未实装**（服务端无路由，且 `apps/miniprogram` 与 `apps/admin-web` 全仓 **0 引用**）—— 缺口在**云资源**：COS 临时密钥签发需存储桶与密钥，随云资源到位 + 「我的」页头像功能一并补 | `{ credentials, bucket, region, expiredAt }` |
...
```json
{
  ...
  "timeline": [
    ...
    { "node": "delivered",  "at": null, "done": false, "text": "待取餐" },
    { "node": "completed",  "at": null, "done": false, "text": "待确认收货" }
  ],
  "pickup": { "point": "国贸三期 B 座 1 楼大堂", "leaderName": "李明", "leaderPhone": "138****0007", "expectAt": "11:30" }
}
```
...
| # | 方法 | 路径 | 说明 |
| --- | --- | --- | --- |
| U12 | GET | `/me` | ⚠️ **本端点不设 —— 用户信息由 A2 `GET /auth/me` 承担**（M5-10 裁定：同一份数据两个端点必然漂移；`buildingName`/`leaderName` 两个派生只读字段已在 A2 出参补齐）· 保留本行只为**留名**：照表验 U12 会拿到 404，那是**预期**，不是缺陷 |
...
**U19 一期口径（M5-20 · 2026-09-23 · 账号注销 · 零 DDL · 表数仍 27）**

| 项 | 口径 |
| --- | --- |
| 为什么必须做 | 《提审自检清单 v1.2》第 10 条：**小程序必须提供账号注销入口**，缺失即不通过。此前入口**完全不存在**（合规清单里挂着「账号注销入口未实装」），是**提审阻塞项**；本次把它从「登记册里的缺口」变成「可点的页面」 |
| ⭐⭐ **注销不是删除行，是「匿名化 + 停用」** | `ab_user.status = 3`（新增 `UserStatus.CANCELED`）+ `deleted_at = now()`；`nickname` 覆写为 `已注销用户`，`avatar_url` / `phone` / `phone_hash` / `building_id` / `team_leader_id` / `subscribe_flag` 一并清空。**`openid` / `unionid` 刻意保留** —— 微信的 `openid` 是**同一个自然人**的稳定标识，删掉它只会让这个人下次登录**又注册成一个全新账号**（同一手机号/同一微信两套账，比留一行匿名数据更糟）。故口径是「**清空可识别信息、保留身份锚点**」，与「用户注销后不能再登录」由 `status=3` 保证 |
| ⭐⭐ **注销后再次登录 → `20014`，不是重新注册** | `AuthService.login()` 在**查库后**先判 `status === CANCELED`（**排在黑名单 `20006` 之前**）→ 抛 `20014`「该账号已注销，如需恢复请联系客服」。**不复用 `20006`**：黑名单是**平台的决定**、注销是**用户自己的决定**，两种成因混成一个码，客服就无法从错误码判断该走「解封」还是「恢复账号」流程 |
| ⭐⭐ **三个闸门 fail-closed，且理由逐条下发** | ① **在职团长不能注销**（名下用户在途、佣金在算 —— 退出团长是另一条有资金清算的路径，见 `20008`）；② **余额 / 冻结 > 0 不能注销**（钱会**凭空消失**：注销后账号不可达，而钱在 `ab_balance` 里趴着，谁也取不出来）；③ **存在未终态订单不能注销**（单子还在跑，注销会让它变成**没有主人的订单**）。命中即 `20015`，`data.reasons` 为**原因码数组**（`'leader'` / `'balance'` / `'orders'`，另附 `pendingOrders` 计数），人类可读文案在 `message` 里、**端上直接展示 `message`**（只回一个「暂不能注销」，用户除了反复点没有别的动作可做）。⚠️ **三道闸门全查一遍再报**，不是命中即抛 —— 逐个报会让用户「改一条、再被拦一条」地来回试 |
| ⭐ **二次确认词由服务端裁定** | 请求体 `confirmText` 必须**逐字等于**「注销账号」（常量 `ACCOUNT_CANCEL_CONFIRM_TEXT`，前后端同步声明）。不符 → `10001`。**这不是 UI 洁癖**：注销不可逆，端上自绘的确认弹窗**不构成证据**，而「服务端认这个词」使得**任何调用方**（含未来第三方端）都必须走同一道确认 |
| ⭐ **原子占位，不做「读-再写」** | 与 M5-19 的 U11 同一纪律：`UPDATE ab_user SET status=3, ... WHERE id=:id AND status != 3`，以 `affected = 0` 判归属（并发两路同时注销时**只有一路成立**，另一路拿到 `20014`）。⚠️ 前面那次「查库判 `status`」只是**快路径**（错误码更准、少一次写），**不构成并发防护** —— 两个请求同时读到 `status = 1` 会**双双注销成功**，第二个人拿到的是「刚注销成功」的**假回执**。判据进 `WHERE` 才是真防护 |
| ⭐ **等级 / 费率 / 佣金不受影响** | 注销**不删** `ab_commission` / `ab_balance_log` / `ab_order` 行 —— 财务凭证的留存要求先于「删除个人数据」，而可识别信息已在 `ab_user` 侧清空。注销是**身份层**动作，不触碰资金账 |
| 端上落点 | `pages/account-cancel/account-cancel`（**原生页**，非 web-view）；入口在「我的 → 设置 → 账号注销」（`mine.vue` 的 `SETTINGS_ACTIONS` 第 5 项）。成功后就地切「已注销」终态屏（**不提供返回**），避免返回栈把用户带回一个已失效的「我的」页 |
| 协议同步 | `constants/agreements.ts` §五 注销条款改为指向「我的 → 设置 → 账号注销」自助路径（原文只说「联系客服」，与实现不符即**合规风险**） |

```json
// 请求
{ "confirmText": "注销账号", "reason": "不再使用" }

// 200
{
  "canceledAt": "2026-09-23T11:40:00.000Z",
  "status": 3,
  "clearedFields": ["nickname", "avatarUrl", "phone", "phoneHash", "buildingId", "teamLeaderId", "subscribeFlag"],
  "note": "账号已注销：同一微信号将无法再登录本小程序，个人资料已匿名化。与订单、退款、佣金相关的记录按法律法规要求保留。如需恢复，请联系客服。"
}

// 20015（示例 · reasons 是**原因码数组**，不是「码+文案」对象数组；
//        人类可读文案在 message 里，端上直接展示 message 即可，不自己拼第二份）
{
  "code": 20015,
  "message": "暂不能注销：账户余额（或冻结额）不为 0，请先提现或使用完",
  "data": { "reasons": ["balance"], "pendingOrders": 0 }
}
```

> ⭐ **`note` 与 `clearedFields` 都由服务端下发**：端上不复制第二份文案 —— 否则改了清空范围（或改了「订单记录仍保留」的说法）之后，端上那句话会**继续按旧口径告诉用户**，而两边都不报错。
> ⚠️ `note` 里**必须写清「订单与资金记录依法保留」**：用户会以为「注销=数据全没了」，而客服在后台仍看得到他的历史订单 —— 说法不一致就是**虚假告知**。
...
> ⚠️ **本节 S4–S8 · S10–S14 共 10 条一期未实装**（S1 / S2 / S9 已实装）。供应商一期**无独立系统**，菜品 / 资质 / 结算账户均属**平台端主数据**，由运营在后台维护；供应商端只保留 S1（工作台）· S2（出餐确认）· S9（应付结算自查）三条自有动作线。**这 10 条不会让任何门禁变红**，但外部测试人照本表逐条验会**整批记为 bug** —— 逐条原因与补齐时机见 **§1.8**。
...
| S4 | GET | `/supplier/dishes` | 我的菜品库（M22-01）· ⚠️ **一期未实装**（供应商一期**无独立系统**、共用后台 `:5173`；菜品是**平台端主数据**，运营走 `/admin/dishes` 维护。供应商端 P23 页已改为**如实说明页**，不假装做完） |
| S5 | POST | `/supplier/dishes` | 新增菜品 · ⚠️ **一期未实装**（同 S4：一期无供应商侧菜品写入入口） |
| S6 | PUT | `/supplier/dishes/{id}` | 编辑菜品 · ⚠️ **一期未实装**（同 S4） |
| S7 | POST | `/supplier/dish-applications` | 上架申请（申请参加某日套餐）（M22-02）· ⚠️ **一期未实装**（一期无「供应商报名参加某日套餐」流程；套餐编排由运营在后台完成 D1–D7）。端上 P24 页为如实说明页 |
| S8 | GET | `/supplier/history?page=` | 历史供应记录 + 好评率（M22-03）· ⚠️ **一期未实装**（无实现、**亦无入口** —— 供应商导航 `SUPPLIER_NAV` 未挂该页）；一期「历史」由 S9 结算明细按日承担 |
| **S9** | GET | **`/supplier/settlement?date=`** | **应付结算明细 / 自查**（日明细 + **跨日期待付合计**）（M23-01 · **已实装** M3-9） |
| S10 | GET | `/supplier/settle-account` | **对公结算账户**信息（M23-02）· ⚠️ **一期未实装**（无实现、**亦无入口**）—— 对公账户由运营在后台维护（D28）；供应商侧结算入口只有 S9 |
| S11 | POST | `/supplier/invoices` | 月度开票申请（M23-03）· ⚠️ **一期未实装**（无实现、**亦无入口**）—— 一期开票台账在运营侧 D44（`/admin/finance/invoices` · 派生视图 · 零 DDL），供应商侧申请入口未做 |
| S12 | GET | `/supplier/profile` | 商家资料 + 资质（M24-01）· ⚠️ **一期未实装**（资质由运营在后台审核维护 D26）；端上 P26 页为如实说明页 |
| S13 | PUT | `/supplier/profile` | 更新资料（资质变更需重新审核）· ⚠️ **一期未实装**（同 S12） |
| S14 | GET | `/supplier/agreement` | 供应商协议与结算规则（M24-02）· ⚠️ **一期未实装**（无实现、**亦无入口**；协议正文一期未落地） |
...
| **D69** | GET | **`/admin/stats/dish-rating?range=&date=`** | **口味红黑榜（P1-U2）** —— 按菜聚合 `ab_dish_rating`（好/一般/不好计数 · 差评率 · **本月投诉数** · 红线告警 · 最近差评原因 · `worstThree`「本周最差三道菜」）· 只读 · 与 D49 菜品热度同页第二个 Tab（P35）· 实现口径见下方「U2 实现口径」表 |
...
**U2 实现口径（U20 提交 + D69 聚合 · 新增第 28 张表 `ab_dish_rating` · 新增错误码 30020·30021 · 2026-09-25）**

> 真源：《ABox一盒创业系统地图与开发路线 v1.0》U2「口味评价与菜品迭代闭环」—— P1 验收判据「**能说出本周最差的三道菜**」。四项开工前裁决（2026-09-25）：① 粒度 = **逐菜三键**（整餐一键无法按菜聚合）；② 红黑榜落点 = **P35 菜品热度页第二个 Tab**（零导航变更 · F5 在 P36 加 Tab 有先例）；③ 红线阈值 = **代码常量**（不进 `ab_config`，避开 D-1 fallback 缺陷）；④ **一次提交即定稿、不可改**。
> 前端落点：端上 `pages/order-detail/order-detail.vue` 评价卡（`canRate` 时渲染）；后台 `views/stats/dish-heat.vue` Tab2「口味评价」。

| # | 口径 | 说明 |
| --- | --- | --- |
| 依据 | U2 卡片三句原文 | 「用户吃完饭一键评价，好吃/一般/不好」+「按菜品聚合口味数据，生成红黑榜」+「这道菜本月第几次被投诉」—— 按菜聚合 ⇒ **逐菜打分**（裁决①），「一键」体现在**一屏 4 行点完一次提交**（约十秒），不是整餐一个分 |
| ⭐ 评价粒度 | **逐菜三键**（每道菜 好吃/一般/不好）：1 好吃 / 2 一般 / 3 不好（落库 `ab_dish_rating.rating`）。**未评的菜不落行**（允许跳过 —— 逼用户评满 4 道菜只会换来全「一般」的垃圾数据）；「再点同一档 = 取消」是端上交互，服务端只认提交上来的行 |
| ⭐ 可评状态 | `delivered` / `completed`（`RATING_ALLOWED_STATUSES`）—— **paid/cut_off/cooked/delivering 不许评**：还没吃到，提交的是「预期不满」而非口味反馈，会污染红黑榜（那类走客服/退款通道）。→ `30020` 回带 `status` + `allowed`（端上据此引导「送达后可评价」而不是干报错）；**刻意不复用 `30003`**（状态机拒绝是取消/改单类话术，排查入口不同） |
| ⭐ 一次定稿（裁决④） | 该单已有**任何**评价行即拒绝新提交 → `30021` 回带 `ratedAt`。**刻意不复用 `10006` 幂等重放**（重放返回首次成功结果，而这里是**拒绝** —— 语义相反，端上处理方式完全不同）；聚合口径要稳（改评得处理「旧分撤回新分入账」），后悔个案走客服（与改价/改单同一兜底） |
| 越权与状态判据 | 归属校验与 U10/U11 同判据：`orderNo + userId` 查不到 → `ORDER_NOT_FOUND`（不区分「不存在」与「非本人」，防越权探测订单号） |
| 菜属校验 | `dishId` 必须在该单 `setMealId` 的 `ab_set_meal_item` 内；不在 / 重复提交 → `10001`（请求内容与订单事实不符，不是业务态） |
| ⭐ 名字快照 | 落库时**快照** `dish_name` / `supplier_name` / `slot`（菜改名/下架/软删都不影响历史行；硬删兜底 `菜品#<id>`）—— 红黑榜展示的是**当时吃到的名字** |
| 整批单事务 | 提交的多行**要么全落、要么全不落**（半批落库会让「已评 2 个菜」既不能续评也不能重评 —— 与 30021 联动成死锁态） |
| reason | 可选自由文本 ≤128 字，**服务端 trim 后落库**（空串 → `null`）；只有 `rating=3` 才有意义，但**不强制绑定**（好吃也可写「希望辣一点」—— 用户建议同样值钱） |
| 不写操作日志 | U20 是**用户侧写操作**，不写 `ab_operation_log`（那是后台操作者审计表）；用户动作的留痕就是 `ab_dish_rating` 行本身 |
| U10 详情内嵌评价块 | `detail.rating = { canRate, rated, ratedAt, items[] }`；`canRate = 未评 ∧ 状态可评 ∧ 套餐有菜`。端上评价卡的**唯一状态来源**（不自己根据 status 推导 —— 状态集合将来扩了，端上不跟进就会「该评的不能评」） |
| ⭐ U10 dishes 补 `dishId` | 评价卡的菜键来源（此前只有 name/slot/supplierName —— **没有 id 就没法提交**）；`dishes[].dishId` 与 U20 提交体强绑定 |
| ⭐ 红黑榜区间 | 复用 `resolveStatsRange`（与 D47–D50 同语义：`range` 定长度、`date` 定终点锚、缺省今日）—— **从 StatsService 私有方法上提为纯函数**（D69 要么注入整张看板服务、要么抄一份；抄一份 = 「缺省锚=今日」这类语义两处各自演化） |
| ⭐⭐ 区间切 **出餐日**（`meal_date`） | 红黑榜按「**吃到的日子**」切，不按「评价提交的日子」—— 周五吃的饭周日才评，该算进周五那一周（「本周最差的菜」要对齐「本周吃到的菜」） |
| ⭐⭐ `badCount` 与 `monthBadCount` **分道** | `badCount` = 区间内「不好」总数；`monthBadCount` = 其中**出餐日在锚点月**的条数。**两者必须分开**：月初那几天，「本月投诉」若按区间算会把上月旧账一并计入 → 红线天天误报（e2e §40 用「上月差评进 30d 窗口、不进锚点月」钉死；单测同款） |
| ⭐ 红线 = `monthBadCount ≥ 3` | 阈值 `RATING_RED_LINE_MONTHLY = 3` 是**代码常量**（裁决③：避开 D-1 `fallback` 恒不生效缺陷；「月投诉 ≥3 次该换菜」是经营判断非运营参数）；**出参下发 `redLineThreshold`，端上不写死**（改阈值文案自动跟） |
| 黑榜序 | 投诉次数降序 → 同投诉次数下**样本越少越扎眼**（差评率高者前）→ `dishId` 稳定序（同分不抖动） |
| `worstThree` | 有投诉（`badCount ≥ 1`）的前 3 —— P1 验收判据「说出本周最差的三道菜」的**直接答案**，服务端算好端上直接展示 |
| 最近差评原因 | `recentReasons` 每菜最多 3 条（`RATING_RECENT_REASONS_MAX`），**id 大者在前**（新→旧）—— 红黑榜一行不该被 10 条重复差评刷屏 |
| 空态 | `dishCount=0 / items=[] / worstThree=[]` —— 空**不是错误**（seed 后天然为空），端上渲染「暂无评价」而非报错 |
| 聚合在纯函数 | `aggregateDishRatings()` 导出为**纯函数**（零 IO，输入全量行）：月界 / 排序 / 截断的判据**可单测**（`dish-rating-agg.spec.ts` 6 条 —— F5 的教训：导出时写「供单测」就必须真有人测，否则是空头承诺）；HTTP 路径另由 e2e §40 覆盖 |
| 权限 | D69 类级与 D47–D50 同（含 `viewer` —— 数据看板组同一批角色）；未登录 `10002` / 小程序与供应商 token `10003`；GET 不写操作日志 |
| 参数 | `range ∈ {today,7d,30d}`（非法 → `10001` 不静默回落 · 同 D47 口径）；`date` 为终点锚（YYYY-MM-DD） |
| 载体 | 新表 **`ab_dish_rating`（第 28 张）**：`uk_dish_rating_order_dish`（每单每菜一行 ⇒ 「一次定稿」由唯一键兜底）+ `idx_dish_rating_dish_date`；只增不改（评价是事实记录）；**第 4 支迁移** `1700000000003-dish-rating.ts` |
| 错误码 | 新增 `30020`（状态不可评）/ `30021`（已评不可改）—— 已回写 §九 + 扩展码登记 |

### 6.7 系统管理（M37）

> **实现状态**：`D51–D56` 已实装（M3-1 基座批次）；**`D57–D58` 已实装（M3-10）**；**`D59–D60` 已实装（M3-12）**；`D66` 已实装（M5-16 · 登录落点）。
> **`D67–D68` 已实装（F5 通知触达矩阵补强）** —— 与本页同一落点（P36「通知模板」页的第二个 Tab）。
> ⚠️ **2026-09-25 校正**：本行原写「`D59–D60` 待后续批次」，与**下方 M3-12 小节**及实装（`admin.controller.ts` 的 `@Get('templates')` / `@Put('templates/:id')`）**互相矛盾** —— 属**遗漏更新**（M3-12 落地时未回改本行）。
...
| D67 | GET | `/admin/messages/reach?from=&to=` | **通知到达率回看**（按场景 + 按日聚合 · F5 · 只读 · 与 D59/D60 同页第二个 Tab · 实现口径见下方「F5 实现口径」表） |
| D68 | POST | `/admin/messages/orchestrate` | **通知编排**（按人按场景批量触达 · `dryRun` 缺省 `true` · F5 · **写操作日志** · 同上） |
...
> 验收 `scripts/e2e-m3.mjs` **§24**（47 条断言 · **不依赖下单窗口**）；**F5 补强另见 §39**（26 条 · 编排 + 到达率 + 截单提醒场景）。

| 项 | 口径 |
| --- | --- |
| 依据 | 原型 `P36 系统配置` 的「📨 消息推送策略（微信订阅消息）」卡片，**原型期 6 个场景逐行对应**（原型 5 行 + M4-3 新增 `commission_settled`）。⭐ **2026-09-25（F5）后为 7 个场景**（+ `cutoff_remind`「截单提醒」· 见下方「F5 实现口径」）—— ⚠️ 新增行**一律追加在数组末尾**，以保住「**前 5 行 = 原型**」这一结构的可核对性（该结构被 e2e §24 锚定；插在中间会让「前 5 行与原型逐一对应」当场失效）。三条原文口径：① 用户端**常规订单状态不推送**（避免打扰），退款结果 / 异常属**必推项**；② 团长送达通知走「微信群 + 服务通知」双渠道；③ 初期以**微信群人工通知兜底** |
...
| 种子状态 | 由 `MESSAGE_TEMPLATE_SPECS` **派生**（不存在第二份场景列表）。⭐ **只有 `leader_delivery` 启用**：它含微信群渠道（人工发群只需文案），不依赖尚未申请的微信模板 ID；其余 6 个场景 `enabled=0`（⭐ F5 后由 5 → 6，新增行同样未启用），如实反映「一期没有微信账号」 |
...
**F5 实现口径（D67–D68 · 通知触达矩阵：编排 + 到达率 · 零 DDL · 零新增错误码 · 表数仍 27）**

> 真源：《ABox一盒创业系统地图与开发路线 v1.0》**F5「通知触达矩阵」** —— 原文「已有 `message` 模块，缺的是**按人按场景编排 + 到达率回看**」，P1 验收判据「**截单与取餐提醒的到达率可见**」。
> 落点：`apps/api-server/src/modules/message/` —— `message-orchestrator.service.ts`（编排）/ `message-reach.service.ts`（到达率）/ `message-admin.controller.ts` + `dto/message-f5.dto.ts`；**场景声明仍是单点**（`admin/template/message-template.specs.ts`，本批只加一行）。
> 页面：`apps/admin-web/src/views/system/message-template.vue`（P36 · **Tab2「到达率与编排」**；Tab1 = 既有 D59/D60）。
> 验收 `scripts/e2e-m3.mjs` **§39**（26 条断言 · **不依赖下单窗口** · 订单夹具一律直插 `ab_order`，同 §18–§24 纪律）。

| 项 | 口径 |
| --- | --- |
| ⭐ **定位：补强，不是新建** | `MessageService.notify()` 解决「**一条**通知按场景规则发不发得出去」；本批解决「**一群人**按场景批量触达 + 结果可回看」。两者是**叠加**：编排**逐条调用 `notify()`**，投递规则（场景 / 启用闸门 / 渠道 / 收件人解析 / 落日志）**仍只在 `message.service.ts` 一处**，编排侧不写第二遍 |
| ⭐ 新增第 7 个场景「截单提醒」`cutoff_remind` | 承载真源点名的「**忘了下单**」。`channels=['wechat_subscribe']` · `requestSubscribe=true` · `wiring='live'`（编排端点是**真实投递点**）。⚠️ **新增行一律追加在数组末尾** —— 保住「**前 5 行 = 原型**」这一结构的可核对性（该结构被 e2e §24 锚定；插在中间会让「前 5 行与原型逐一对应」当场失效）。⚠️ 与 `user_order_status` **不是同一件事，别混**：后者是「订单状态流转」，前者是「**目标日尚未下单**」的促单 |
| ⭐⭐ 受众只能是**预定义选择器** | 入参 `audience` 是**受限枚举**（`AUDIENCE_SELECTORS`），**刻意不收 `userId[]`** —— 收一个任意用户列表就等于开了一个「给谁都能发」的**群发后门**；后门一旦存在，就没人再走业务链路，而是直接拼列表发 |
| ⭐⭐ 选择器与场景**强绑定** | `selector.scene === spec.scene` 强校验，串用 → `10001`。防的是「拿退款场景的模板给截单受众群发」 |
| ⭐⭐ `dryRun` 缺省 `true`（fail-safe） | 编排是**会被误触**的动作（一个 POST），而一次能影响几十上百人 ⇒ 默认**只算不发**，要真发必须显式传 `dryRun: false`。与「启用闸门 fail-closed」同一取向：**让人多按一次，好过错发一次**。⚠️ 默认值定义在**服务层**（`input.dryRun !== false`），DTO 里**刻意不给默认值** —— 否则「未传」与「传 `false`」在契约上不可分 |
| ⭐ 受众解析的「差集」在**服务端内存**完成 | SQL 里 `NOT IN (子查询)` 与大 `IN (...)`（SQLite 绑定变量上限 999）**跨驱动行为不一致** ⇒ 两次简单查询 + 内存 `Set` 差集（同 D44 发票先例）。判据：**排除 `pending_pay` / `cancelled`** —— 这两态表示**没有真正买过**，拿来当「有购买习惯的人」会得到一份虚高名单；其余状态含 `refunded` **保留**（退过款的人仍是「曾下过单的人」，是否再触达属运营判断，不是系统该替他过滤的） |
| ⭐ 受众解析**不按 `openid` 过滤** | 缺 `openid` 的人在投递时进 `skipped[]`（原因「收件人 openid 缺失」，见 `MessageService`）—— **不静默**。代价是 `audienceSize` 会把「收不到的人」也算进去；⚠️ 当前**无实际影响**：一期种子用户 `openid` 为 `mock_openid_100x`（**全覆盖**），演示用户 `demo_abox_%` 亦然。若将来出现无 `openid` 的真实用户，此处的口径需要在**一处**重新裁定 |
| ⭐⭐ `ab_message` **没有 `scene` 列** | 场景藏在 `payload.scene`（JSON 文本）。`JSON_EXTRACT` 属**驱动相关语法** ⇒ 到达率的**按场景聚合必须在服务端内存完成**（同 D44 纪律），不能下推 SQL |
| ⭐⭐ 到达率**分母口径** | `reachRate = 投递成功数 / **已尝试投递数**`（**不是** `/ 受众数`）—— 分母**不含**「因未启用而跳过的人」。若按受众数算，一期整页恒为 0，且分不清「没人该收」与「发了全失败」 |
| ⭐⭐ 无记录 → `reachRate` 是 **`null`，不是 `0`** | 「区间内没发过」与「发了全失败」是两回事。显示成 0% 会让运营去查一个**不存在的故障**；端上 `rateText()` 把 `null` 渲染为 `—` |
| ⭐ 无记录时 `note` **明写原因** | 一期无微信账号 → 模板 ID 全空 → 启用闸门拦住 → 投递侧**跳过且不写日志**。「0 条」与「接口坏了」必须能分开读 |
| ⭐ `scenes[]` **含 0 投递的场景**（全部 7 个） | 否则一期整页空白，运营看不到「有哪些场景、各自什么状态」。另出 `orphanScenes[]`（已下线场景的历史记录）—— **不静默丢** |
| 区间口径 | 缺省**近 7 天**；`MAX_RANGE_DAYS = 90`（= `ab_message` 的保留期）。**非法日期 / `from > to` / 超 90 天一律 `10001`，不静默回落** —— 回落会让运营看一份「答的不是他问题」的报表，而没有任何一处提示「你的入参被忽略了」 |
| ⭐ 范围过滤走 `created_at`（非空列），**日归属优先 `sent_at`** | 两者**不可混用**：用 `sent_at` 做范围过滤会把「已尝试但未成功」（`sent_at` 为空）整批漏掉 —— 而它们正是到达率**最该被看见**的部分 |
| 时间列存 UTC | 一律经 `toBjIso()` 转北京时区后再按自然日归属（本项目所有时间列**存 UTC**；直接与库内 `'YYYY-MM-DD HH:mm:ss'` 文本比较会整体偏 8 小时） |
| ⭐ `NOTIFY_PAGES` 新增 `orderCreate` | 促单要落到**能下单的页**（`pages/order-create/order-create`），而不是订单详情 —— 提醒的价值在「点进去就能下单」 |
| ⭐ `wxData` 的 key 用**我们自己的变量名** | 真实微信模板要求 `thing1` / `time2` 这类 keyword，而其由**微信公众平台侧创建模板时**决定，一期拿不到。凭空造 keyword 属**编造契约** ⇒ 照 `refund.service.ts` 既有做法，映射待模板创建后在**一处**补齐 |
| ⭐ 独立控制器 `MessageAdminController` | 与 `admin.controller.ts`（`@Controller('admin/system')`）**域不同**（消息域 vs 系统管理域），且**权限白名单只声明一次**（类级 `@Roles('super_admin','admin')`）。**但同属 P36 一页的两个 Tab** ——「同一页」是**前端信息架构**，「两个控制器」是**后端域划分**，两者不必一致 |
| 操作日志 | D68 **写**：`@OperationLog({ module:'message', action:'编排通知触达' })` —— 批量触达必须能回答「谁在什么时候给哪群人发了什么」；D67 **只读，刻意不加** `@OperationLog()`（同 D56/D66 的 GET 纪律） |
| 权限 | 类级 `@Roles('super_admin','admin')`。`operator` / `finance` / `viewer` / `supplier` / 小程序 token → `10003`；未登录 → `10002`。编排是**对外产生影响的动作**，不能由用户侧触发 |
| 错误码 | **无新增**：场景不存在 / 受众选择器不存在 / 二者串用 / 日期非法 / 区间超限 → `10001`（**全部 fail-closed，不静默回落**） |
| ⭐ **结构性不变式** | `requestSubscribe === true` 的场景**必须** `wiring === 'live'` —— 否则就是「**索权不用**」：向用户要一个我们根本不会用的授权，微信平台明确反对。e2e §39 对该不变式做**全量断言**（不是只查新增那一行） |
| ⚠️ 一期**必然一条也发不出去** | 无微信账号 ⇒ `wechat_template_id` 全空 ⇒ 启用闸门拦住所有订阅消息场景 ⇒ `notify()` 在第 2 步返回 `delivered=false`。**这不是缺陷，是当前真实状态**：出参照样给出真实 `audienceSize`，让运营看得见「今天若已开通，会有多少人收到」。**不许把它粉饰成「已发送」** |
| ⚠️ **未做的部分（如实登记缺口，不假装已解决）** | ① 三通道中的**企业微信 / 短信**两条**一期不做**（2026-09-25 裁决）→ 已入《悬而未决登记册》。② **自动跑批**未接入（真源 F5 的「依赖：订阅消息模板审核」）—— 一期触达入口是**编排端点**（`POST /admin/messages/orchestrate`）；接跑批时**直接调 `MessageOrchestratorService.orchestrate()`**，业务逻辑一行不改（同「跑批与手动补跑共用同一执行口」纪律，见 §6.8 `ScheduleService.run()`）。③ 到达率页的**趋势图**未做（一期只出表格 + 摘要） |
...
| P5/P6/P7 | 订单详情/列表/取消 | M03 | U9/U10/U11 + **U20（P1-U2 · 详情页内嵌评价卡）** | `ab_order`、`ab_refund`、`ab_dish_rating` |
...
| P35 | 数据看板 | M36 | D47–D50（**已实装**）+ **D69（P1-U2 · 第二个 Tab「口味评价」红黑榜）** | 聚合查询、`ab_dish_rating` |
| P36 | 系统配置 | M37 | D51–D60 · **D67–D68（F5 通知触达矩阵：编排 + 到达率 · 同一页第二个 Tab）**（**已实装**） | `ab_admin_user`、`ab_operation_log`、`ab_config`、`ab_message`、**`ab_message_template`** |
...
| **20014** | 该账号已注销，如需恢复请联系客服 | U19 注销账号后再登录（或对已注销账号再调 `/me/cancel`）—— ⚠️ **刻意不复用 `20006` 黑名单码**：黑名单是**平台的决定**、注销是**用户自己的决定**，混成一个码则客服无法从错误码判断该走「解封」还是「恢复账号」流程（**扩展**） | 403 |
| **20015** | 存在未结清事项，暂不能注销 | U19 三个前置闸门任一命中（在职团长 / 余额或冻结 > 0 / 有未终态订单）；`data.reasons` 下发原因码数组（`leader`/`balance`/`orders`）+ `data.pendingOrders`。⭐ **fail-closed 的正确含义是把用户挡在一次失败上，而不是「连余额一起清掉」** —— 后者 = 用户的钱被系统吞掉且**不会有任何报错**（**扩展**） | 409 |
...
| **30020** | 当前订单状态还不能评价（送达后可评价） | U20 提交口味评价时订单状态 ∉ {delivered, completed} · 回带 `data.status`（当前状态）+ `data.allowed`（可评状态集）—— 端上据此给出「送达后可评价」的引导而不是干报错。⚠️ **刻意不复用 `30003`**：状态机拒绝是「取消/改单」类话术，评价的排查入口是「订单走到哪一步了」（**扩展**） | 409 |
| **30021** | 该订单已评价过，不能重复提交（一次提交即定稿） | U20 对已有评价行的订单再提交 · 回带 `data.ratedAt` —— 端上直接切「已评价」终态。⚠️ **刻意不复用 `10006` 幂等重放**：重放返回首次**成功结果**（可当成功吞掉），这里是**拒绝**新提交 —— 两者对端上的处理方式完全相反（**扩展**） | 409 |
...
> **扩展码**：`20006` / `20007` / `20008` / `20009` / `20010` / `20011` / `20012` / `20013` / `20014` / `20015` / `30008` / `30009` / `30010` / `30011` / `30012` / `30013` / `30014` / `30015` / `30016` / `30017` / `30018` / `30019` / `30020` / `30021` / `40009` / `40010` / `40011` / `40012` / `40013` / `40014` / `40015` / `40016` / `40017` / `50003` / `50005` / `50006` / `50007` / `50008` / `50009` / `50010` / `50011` / `50012` / `50013` / `60001` / `60002` / `60003` / `60004` / `60005` 号段内文档原未列、但工程实现需要，已按「号段末尾登记」规则回写本表（见 `apps/api-server/src/common/constants/error-code.ts` 头部纪律）。**禁止挪用文档已占用的号位。**

> **号段划分**：`1xxxx` 通用 · `2xxxx` 账号/团长身份 · `3xxxx` 套餐与订单 · `4xxxx` 支付与退款 · `5xxxx` 供应商与集散 · **`6xxxx` 主数据（办公楼 / 楼群）** · `9xxxx` 系统。

> **前端契约**：`code` 为**稳定标识**，文案可迭代；端上对 `10002`（跳登录）、`30001`（截单提示）、`40004`（引导联系团长）、**`20014`（账号已注销 —— 清本地登录态并就地提示，不跳登录页，否则用户会在「登录→被拒→跳登录页」之间死循环）** 做特殊分支处理。
