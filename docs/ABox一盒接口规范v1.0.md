...
**S9 应付结算口径（C9 · 2026-09-14 定案）**：单价 = 该供应商当日供应菜品的**菜品成本单价**（如红烧肉 ¥7.50/份）× 实发份数；集散中心另按 **¥5.00/份**（米饭 ¥2.00 + 打包 ¥3.00）单列。**本接口只做「应付金额计算与状态展示」—— 实际付款由财务走人工对公转账，系统不做任何支付通道分账调用**，故状态是 `待付 / 已付 / 已冲减`，而非「分账成功 / 失败」。
...
> **C4 纪律**：集散中心**不是硬编码 4 个**，而是 `ab_distribution_center` 表驱动、默认 4 个、可增删；分账参数（米饭 ¥2.00 / 打包 ¥3.00）存 `ab_config`，管理员可调。
