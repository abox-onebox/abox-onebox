...
| 最近修订（M5-8） | 2026-09-18 · **履约链 T7/T8/T9 补实现（收口缺陷 #79 · 零 DDL）**：① ⭐⭐ **新增 §6.5 D63 `PATCH /admin/deliveries/{id}/status`** —— `ab_order` 的 `cooked`/`delivering`/`delivered` 三态**全仓零写入点**（订单支付后永远停在 `cut_off`、团长「确认取餐」永远返回零值**且不报错**、**佣金永不产生**、自动确认跑批每天把全部订单报成「履约异常」），而当时 **19 道门禁 + 969 条 e2e 全绿**（夹具直接 `UPDATE ab_order SET status='delivered'` 造数据 → 测试永远从链路**中间**开始）；现补上 T7（`SupplierService.advanceOrdersToCooked`，出餐确认**同事务**联动）与 T8/T9（D63）；② ⭐⭐ **「改数字」（D62 `PUT`）与「推进履约」（D63 `PATCH :id/status`）刻意分成两个端点** —— 合成一个会让「份数 5→3 顺便把状态也推了」变成一条记录两件事，审计上分不清责任；③ ⭐ **订单联动是「尽力而为 + 如实报告」**：不做「订单必须都在 `cooked`」的前置校验（会把运营卡在门口、然后绕过系统打电话），能推的推、推不动的计入 `orderTransition.remaining` 并给可照着排查的 `note`；④ ⭐ **`called`（已叫车）不动订单**（货还在加工场所），出参 `orderTransition=null` 且 `note` 写明「这不是漏了联动」；⑤ ⭐ **D61 列表每行新增 `orderStatusBreakdown`**（该楼群当日订单状态分布，口径与 `orderQuantity` 同源）—— 让「有几单没跟上」从**一个看不见的洞**变成**列表里的一行**；⑥ ⭐ **T9 的取餐通知不装样子**：场景 `leader_delivery` 一期渠道就是**微信群**（`wiring='pending'`），往 `wechat_group` 调 `notify()` 会被判「无程序投递点」跳过 —— 加一个必然跳过的调用点只会让接线状态变成**假的 `live`**，故出参明确写「需运营人工发群」；⑦ **新增错误码 `30018` `DELIVERY_STATUS_ILLEGAL`**（带 `data.allowed` · **刻意不复用 30016** —— 排查入口不同）；⑧ ⭐⭐ **门禁 `state:audit` 收紧至豁免 0** 并新增**规则④「不可达态必须显式登记」**（`ORDER_RESERVED_STATUSES` 登记 `refunding`），同时**删掉那条躺了三个批次的不可达边** `refund_applying → refunding`（M4-3 起订单审批通过一步到 `refunded`，通道进度交 `ab_refund.status`）；⚠️ 该门禁经 **5/5 反证探针**打红过（删写入点 / 恢复旧边 / 取消保留态 / 塞孤儿态 / 把判定函数改恒空）；⑨ e2e **新增 §35 · 27 条断言 · 全链路**（U6 下单 → 支付 → 截单 → 配送单生成 → 出餐确认 T7 → 已叫车 → 配送中 T8 → 已送达 T9 → 自动确认 T11 → 佣金入账 4.5，**除一处夹具外订单每次状态变化都来自真实 HTTP**）；⑩ ⭐⭐ **闸门③ 由「只能向前」收紧为「一次只能一步」**（`allowed` 只含紧邻下一态）—— 跳级（`called → arrived`）会**跳过 T8**，而 T9 的条件更新是 `delivering → delivered`，订单还在 `cooked` ⇒ **一单不动且不报错**（正是 #79 的形状）；⚠️ 这是实现自查出的**第二处「两份表述」漂移**（注释/文档写「跳级一律拒」而 `slice(fromIdx+1)` 实际放行），已补 e2e 断言 `allowed === ["en_route"]`；⑪ ⭐ **后台入口补齐**（`admin-web` 配送页「推进到 X」按钮 + 确认弹窗 + 「订单进度」列）—— **只有接口没有入口 = 能力存在但无人能到达**（与 #79 同族） |
| 最近修订（M5-14） | 2026-09-19 · **U5「今日这盒 · 商家溯源」由空壳补实装（零 DDL · 零新增错误码 · 表数仍 27）**：① ⭐⭐ **§3.2 整节重写为实装契约**（原节是**未实现时的设计稿**，与实装有三处不一致，逐条纠正 —— 均属「同一件事两份表述」缺陷族）：⚠️ **ⓐ 资质项由三项改两项**（删除 `health_cert`）—— 自营裁定后健康证的主体是 **ABox 自有持证场所**（热加工 / 打包）而非半成品供货方，且 `ab_supplier` **无对应列**，照抄原型三分项会构成**食品安全叙事上的虚报资质**；⚠️ **ⓑ `takeoutLinks` 由顶层字段改为挂在 `dish.supplier` 下**（跳转入口是「这家店」的，不是「这一餐」的 —— 顶层摆放会让端上被迫把「哪家供应商配哪个链接」重新推一遍，而它拿不到 `supplierId`）；⚠️ **ⓒ 补 `setName` / `recommended` / `configured` 三字段**（`recommended` 决定「推荐」角标、`configured` 决定置灰 —— 二者缺失则「缺京东」这件事在端上**不可表达**）；② ⭐⭐ **资质判据收紧为「在册且已核验」** —— 判据是 `ab_supplier.audit_status='approved'`，**不是「证照字段非空」**：「上传了文件」与「平台核验过」是两件事，未过审 ⇒ **空数组**（端上不显示核验行）；③ ⭐⭐ **`traceNote` 由「本次真实资质并集」生成**（服务端下发、端上不拼）—— 刻意不硬编码「三重核验」文案，否则数据一变声明就继续说着旧话；④ ⭐ **三平台入口恒返回 3 条**（顺序固定），`url=null` + `configured=false` = 该平台未入驻，是**合法状态**：端上**置灰而不隐藏**（缺平台是要去谈的，不是不存在的）；⑤ ⭐ **悬空推荐读侧兜底** —— 写侧（D33）已挡「推荐了但没配链接」，但直接改库 / 改种子 / 历史数据都能绕过写侧，故**读侧再校验一次**，保证端上永远拿不到点了没反应的入口；⑥ ⭐ **未开团不报错**，回 `dishes: []` + 说明文案（与 U1 的 `MEAL_NOT_PUBLISHED` 选择**相反**：U1 是交易页、报错能说清「为什么不能下单」；U5 是信任页、无团可溯时用户无从 action，红色 toast 只会让人以为坏了）；⑦ ⭐ **`buildingId` 必传**（`@Public()` 下守卫不注入 `req.user`、无从推楼群，且**不做「回落到任一楼群」的猜测**）；⑧ ⭐ **§八 映射表 P38 行纠正** —— 「主要表」原写 `ab_supplier_dish_daily`（该表是**出餐确认**用的），实装读的是套餐侧（`ab_meal_assignment` / `ab_set_meal` / `ab_set_meal_item` / `ab_dish` / `ab_supplier.takeout_links` / `ab_distribution_center`），又是一处文档↔代码漂移；⑨ ⚠️ **早期承诺的 Redis 缓存如实改标「一期未实装」**（本仓库 Redis 仅作队列驱动、无缓存层；溯源低频只读且查询已收敛为**常数 4 次**，与菜数无关）；⑩ 抽取 **`common/utils/takeout.ts`** 作 `takeout_links` 的**唯一读取口**（D33 后台配置页与 U5 共用），避免同一列 JSON 被两处各解析一遍 —— 那正是「后台显示已入驻、端上却是灰的」且**两边都不报错**的成因。 |
| 最近修订（M5-16） | 2026-09-20 · **后台工作台待办聚合 D66（零 DDL · 零新增错误码 · 零新状态 · 表数仍 27）**：① ⭐⭐ **§6.7 新增 `D66 GET /admin/dashboard/todos`**（登录落点 `/dashboard` 的**唯一数据源**）—— 出参 `items[]`（**4 条固定 key、顺序即界面顺序**）+ `brief`（今日作业 4 指标）+ `businessDate` / `tomorrowDate`；② ⭐⭐ **四条计数的口径全部复用既有状态机**（**不引入任何新状态** —— 新状态 = 新的「两份真相」起点）：`refundApplying` ↔ `ab_refund.status='applying'`（与 D40 待审同一集合）· `withdrawPending` ↔ `ab_withdraw.status='pending'`（与 D45 同一集合）· `tomorrowGroupsUnassigned` ↔ 「在营楼群」(`ab_building_group` status=1 且 `deletedAt IS NULL`) 减「明日有未取消分配」(`ab_meal_assignment.status != 'cancelled'` —— ⚠️ **与 D6 `usedCount` 同源，`!= 'cancelled'` 不可省**，否则**停团楼群被算成已排**、卡片恒显 0 且不报错）· `deliveriesLate` ↔ `ab_delivery_record.status != 'arrived'`；③ ⭐⭐ **`deliveriesLate` 带时间闸门** —— 仅在**当日送达时刻已过**（`now() > arrivalAtOf(today)`）时才查库计数，**送达前恒 0 且不发 SQL**（否则「还没到点」会被误报成「已延误」，一张每天早上必然亮起的假待办会把人训练成无视它）；④ ⭐ **`count=0` 也下发**（端上按「有活 / 已清空」两态渲染，**不是整卡消失** —— 消失会让人以为「这个功能没做」）；⑤ ⭐ **`hint` 由服务端下发**（端上不复制第二份口径文案）；⑥ ⭐ **角色：含 `viewer`**（与 D8–D12 的类级白名单不同 —— 工作台是**只读聚合**，正是 viewer 的天然落点）；供应商 token / 小程序 token → `10003`、未登录 → `10002`；**GET 只读，刻意不加 `@OperationLog()`**；⑦ ⭐ **端上按 `account.menus[]` 过滤卡片**（`perm.visiblePaths`）—— 那是**渲染纪律**，不是安全边界（边界在守卫）；⑧ e2e **§38 · 20 条断言**（结构 5 + ①②③③b 逐条对账 4 + 闸门与夹具收尾 5 + 角色矩阵 6），m3 **1035 → 1055**。详见《里程碑计划》§五「子批次 M5-16」 |
| 最近修订（2026-09-20 · 三处严重问题排查修复批次） | 2026-09-20 · **身份一致性 / 套餐模板编辑 / 一饭四菜机械校验 / 菜单门禁（零 DDL · 表数仍 27 · 新增 1 错误码 `30019`）**：① ⭐⭐ **身份一致性（排查①）** —— `leaderName` 不再「未跟随却自回退成本人」（`buildingId` 的独立回退保留）；退出团长后**重新拉取 A2 并回写本地**（否则端上仍显示旧身份）；个人中心昵称改**以服务端快照为准**（服务端是真相，本地缓存会滞后）。并修掉测试环境的**根因**：`constants/env.ts` 硬编码 `dev:1001` ⇒ 测试时**无法切换身份**，新增 **`?devCode=1002` 落地即切换**（`utils/dev-identity.ts` + `abox_dev_identity` 存储 + `App.vue onLaunch` 读取并清态）。⚠️ 一期口径：**「切换身份」= 切换登录用户**，不是同一用户的多角色视图 —— 身份模型只有两个主体（`ab_user` 与 `ab_admin_user`），**团长是 `ab_user` 的叠加身份**，故「用户 ↔ 团长」本就在同一账号内切换、无需换人。② ⭐⭐ **套餐模板补编辑入口 D7b（排查③）** —— 原只有 D7「存为模板」，模板建出来**只能改状态、不能改内容**（名字打错 / 想换一道素菜都只能新建）；补 `PUT /admin/meal/templates/{id}`（**不传=不改 · 显式 `null`=清空 · `items` 整组替换**），并定 **引用冻结** 语义：`usedCount > 0` 时 **items / price 拒改 → 新码 `30019`**（出参带 `usedCount` / `assignmentDates`），`name` / `oneLiner` / `description` / `status` **仍可改** —— 冻结**范围**本身就是口径（名字给人看，items/price 给账与出餐看）；`status` 上下架不受此限（紧急下架是安全阀，同 D4 不加闸）。③ ⭐⭐ **一饭四菜由文案升为机械校验** —— 此前该约束只活在文案与种子里（运营能建出「三道素菜」的套餐，用户端照卖 ⇒ **约束不落成代码 = 约束不存在**）；真源收敛到 `@abox/shared-utils` 的 **`SET_MEAL_COMPOSITION`**（`requiredSlots=[1,2,3,4]` · `stapleSlot=5` **主食米饭由集散中心统一供、不建 `ab_set_meal_item`** · `rule`）与 `SET_MEAL_SLOT_LABEL`，**校验顺序 = ①重复档位 `10001` → ②菜品不存在/已下架 `30008` → ③构成不合 `10001`**，`GET /admin/meal/dishes` 出参**同源下发** `slots[].required` / `slots[].isStaple` + `composition`，后台编排页**只渲染 4 个蔬菜档位**、主食以只读提示呈现。④ ⭐⭐ **菜单授权 ↔ 入口一致性门禁（排查②③）** —— 该类缺陷**已复发 3 次**（M3-14 财务五页 → M5-12 通知模板 → 本次 building 四页），补 `scripts/check-nav-consistency.mjs` 并纳入 `gate.mjs all`（**16 → 18 道**）；**首次运行即抓出 5 处历史遗留**：`/supplier/packing-center` 已授权且控制器存在却**缺席 `ADMIN_MENU_KEYS`**（授权了却点不到 = 死入口）· 4 个统计页（`core-metrics` / `building-rank` / `dish-heat` / `retention`）在 `routes.ts` 可达却**未进 `ADMIN_NAV`**（无入口）· `/leader/apply` 属**刻意下钻**（登记进豁免表，不算断链）。⑤ ⭐ **停职复职改名留痕** —— `apply()` 复用同一条 `ab_team_leader` 时，若 `realName` 变更则**同事务写 `ab_operation_log`**（`module='leader'` · `action='复职团长'` · `snapshot={before,after}`）：佣金/订单/审计均按 `leader.id` 关联，改名会让**历史记录上的人名跟着变**，必须可回溯。⑥ ⭐ **三处占位页补实** —— 后台「运营概览」由 `el-empty` 改为**按账号可见菜单渲染的导航页**（零 API）；供应商端「我的菜品 / 资料」改为**如实说明页**（写明一期未实装、缺口登记在《悬而未决登记册》）—— 挂「待开发」标签的假页比空白更误导。⑦ e2e **§14 由 2 菜品断言改为 4 菜品一饭四菜** + 新增缺档 / 超档 / 重复 / 已下架断言 + **D7b 全组**（未占用可改 · 占用改 items/price → `30019` · meta+status 仍可改 · `30008` · 空体 `10001`）；m3 **1026 → 1035**（+9）· `gate.mjs all verify` **22/22 全绿**（本批新增**两道门禁**：`nav:consistency` 菜单「授权 ↔ 入口」一致性 + `doc:tables` **契约文档表格完整性** —— 后者首跑即扫出**表格空行截断 5 处存量**，**本文件命中 1 处**：「最近修订（M5-14）」整行**粘在 M5-8 行尾**，渲染时该行多出两列）。 |
| 最近修订（M5-17） | 2026-09-20 · **首页三处调整 + U1/U5 编号与归属两处补字段（零 DDL · 零新增错误码 · 零新表 · 表数仍 27）**：① ⭐⭐ **U1 `dishes[].supplierId`（新）** —— 端上「来自：X」由纯展示变为**可点**，跳 `P38` 溯源页并**按 id 定位**到那家（描金圈 + 直接弹出平台层）；**不按 `supplierName` 定位**：`ab_supplier.name` **无唯一约束**（同名两家是合法数据），按名定位会让用户在溯源页**照着一家不是自己点的那家去平台下单** —— 这是该页最坏的一类错误。⚠️ 与 C8 不冲突：id 既不是合作状态、也不是供价 / 分账 / 联系方式，且**只在「已能看见该供应商展示名」的接口里出现**；② ⭐⭐ **U5 `supplier.id`（新）** —— 同理，供端上按 id 定位；两接口的编号**必须同源**，e2e 不满足于「字段存在」而是**逐菜对账**（同菜名 ⇒ 同 id、同展示名）；③ ⭐⭐ **U1 `leader.source`（新）** —— `'bound'`（经邀请链接绑定）/ `'building_default'`（未绑定 → **自动挂靠**本楼在任团长）。用户可能从未走过邀请流程，系统按本楼在任团长自动挂靠、**佣金也归那位** —— 这件事必须写在脸上：佣金是**从这一单里分出去的**，让用户以为「我没跟谁」而实际有人在收他的佣金，是比不显示更糟的沉默。端上在**首页团长卡**与**下单确认页的取餐信息卡**两处明示（后者是付款前最后一次能看见归属的机会）；④ ⭐⭐ **收口缺陷⑫：U1 与 U6 的归属判据一致化** —— U1 的 `leader` 只决定**显示什么**，`OrderService.resolveLeader` 决定**钱归谁**，两者此前判据不同（U6 要求绑定团长**在职**、U1 不看）。于是「团长停职 + 用户未重绑」时首页写着 A、佣金却记给 B。根因是 D22「停职」只改 `ab_team_leader.status`、**不清** `ab_user.team_leader_id`（**这是对的**，复职后绑定该还在）⇒「绑定行」与「有效归属」本来就可以不是同一个。修法是**让显示侧照抄交易侧的顺序**（①绑定且在职 → ②本楼 id 最小且在职 → ③`null`）并额外下发 `source`，而**不是**让两边各写一份判据；⑤ ⭐⭐ **e2e 不许「拿 U1 自己当期望」** —— 缺陷⑫ 的回归用**停职**把两侧逼到分歧点，期望值由**独立复现的 U6 判据**（直查 `ab_user` / `ab_team_leader` 的同一段 SQL）给出：若拿 U1 出参当期望，这条断言只能证明「U1 等于它自己」，对分歧完全无感（本项目反复踩的**假绿**形状）。停职样本**断言成败都复位**，避免污染后续用例与下次种子；⑥ ⭐ **端上首页精简**：移除页尾「往日这盒」（U2）—— 首页只保留「今天订不订」这一件事，每多一块主按钮就往下掉一屏；**接口与端上封装 `fetchHistory` 均保留**（M1 既有验收点，且日后做「历史」入口可直接复用），只是不再占首页版位；⑦ ⭐ **「谁先动手」的顺序决定**：本批**先改契约、后改端上**（端上依赖新字段），且**没有**把平台弹层抽成共享组件 —— 首页只传 `supplierId`，跳过去的是**另一个页面实例**，数据由 U5 现取；把上一页的 `supplier` 对象塞过去会造出「页面显示 A、实际数据是 B」的**第二种真相**；⑧ ⭐ **新增 `ab-supplier-card` 的 `highlight` prop**（而非由页面给组件挂 `class`）：组件有自己的根节点与 scoped 样式，外部 `class` 是否落到根节点上在各小程序端表现并不一致 —— 「看起来生效、真机不生效」正是本项目最想避免的缺陷形状，故用 prop 把这件事变成确定行为；⑨ e2e **m1 54 → 64**（新增 10 条：编号同源 2 · `source` 两态 2 · `supplierId` 正整数 1 · 停职回归 3 · 停职样本构造与复位 2）。详见《里程碑计划》§五「子批次 M5-17」 |
...
**⭐ 身份模型（2026-09-20 · M5-15 补 · 勿推翻）**

| 项 | 口径 | 为什么 |
| --- | --- | --- |
| 主体只有**两个** | `ab_user`（用户端 · **与团长共用同一行**）· `ab_admin_user`（后台 · **含供应商**） | 「团长」**不是**第三种主体，而是 `ab_user` 上的一张 `ab_team_leader` 记录（**叠加身份**）；「团员」= `ab_user` + `team_leader_id` 有值。三者是**同一行的不同视图**，不是三张表的三种人 |
| 一期**「切换身份」= 切换登录用户** | 不提供「同一账号的多角色视图切换」 | 用户 ↔ 团长在**同一账号内**切换（叠加/退出），**不涉及换人**；只有「测另一个人」才需要换登录用户 |
| 退出团长 | **只删 `ab_team_leader` 记录**，`ab_user` 与历史订单/佣金**全部保留**（`leader.id` 是它们的关联键） | 退出的是**身份**，不是**人**。删用户会让历史记录失去主体 |
| `leaderName` 回退 | **只认 `ab_user.team_leader_id`**，未跟随即回显「未绑定」；**不回落成「本人」** | 回落成本人会让**退出团长的人仍在自己名下看到自己**（M5-15 排查①）。`buildingId` 的回退**另算**（P8 的诉求由它独立满足）—— 两个字段**语义不同，不共用开关** |
| 服务端不信任 token 的 `isLeader` | 每个团长端点**实时查** `ab_team_leader.status` | 停职/退出后 token 仍有效 ⇒ 靠 token 声明就有**权限窗口**；实时查库是零窗口 |

> **本地联调的身份切换（`?devCode=` · 仅 demo 模式）**
>
> 端上取登录 code 只有一条路：`fetchWxCode()` → `ENV.wxDevLoginCode`（出厂 `dev:1001`）。
> 于是**任何入口、任何设备、任何一次冷启动都是同一个人** —— 人工测试想验「普通用户 / 团员 / 团长」时，
> 怎么退出、怎么换用户名，拿回来的还是 1001（现场被描述成「换了用户名，'我的'页仍显示旧身份」）。
> **这是测试环境限制，不是服务端缺陷**：mock provider 本就支持 `dev:<任意标识>`（`/^dev:(.+)$/`），缺的只是**端上把它透出来**。
>
> 用法：H5 `http://<ip>:5180/?devCode=1002` · 开发者工具给页面配 query `devCode=1002`。
> 换 `devCode` 即换人，端上会**清掉上一个人的登录态**再登录（否则旧 token 会被 `restore()` 当成「已登录」）。
>
> ⚠️ **安全边界**：`applyDevIdentity()` 与读取方 `fetchWxCode()` **均在 `ENV.demoMode` 分支内**，
> `demoMode = MODE !== 'production'` ⇒ **生产构建下本机制整体不生效**（`devCode` 从不被读、内部再自检一次拒绝写入）；
> 且 `devCode` 走**白名单式校验**（`^[A-Za-z0-9_-]{1,32}$`），不接受任意串注入。**这不是线上可指定任意用户的口子。**
...
| # | 方法 | 路径 | 说明 | 关键出参 |
| --- | --- | --- | --- | --- |
| U1 | GET | `/home/daily` | 明日套餐 | `{ mealDate, publishAt, cutoffAt, canOrder, countdownSec, dishes:[{name,image,category,supplierName,supplierId}], rice, priceFen:2580, leader:{id,name,building,floor,source} }` |
| U2 | GET | `/home/history` | 历史套餐归档（M01-02） | 分页 `{mealDate, dishes[], priceFen, ordersCount}` · ⚠️ **端上首页已不展示**（M5-17），接口保留 |
...
| 字段 | 口径 |
| --- | --- |
| `dishes[].supplierId` | 出品方 id（`ab_supplier.id`），**恒为正整数**。存在的唯一理由：端上「来自：X」这一行可点，跳溯源页并**按 id 定位**。**不按 `supplierName` 定位** —— `ab_supplier.name` **没有唯一约束**（同名两家是合法数据），按名定位会跳错家。⚠️ 与 C8 不冲突：id 既不是合作状态、也不是供价 / 分账 / 联系方式 |
| `leader.source` | `'bound'` = 经团长邀请链接绑定（`ab_user.team_leader_id` 指向的**在任**团长）；`'building_default'` = 未绑定 → **自动挂靠**本楼在任团长（`ab_team_leader` 中该楼 id 最小且在职者）。端上据此把「你的邀请团长」与「本楼自动挂靠」**说清楚** —— 佣金是**从这一单里分出去的**，让用户以为「我没跟谁」而实际有人在收他的佣金，是比不显示更糟的沉默 |
| `leader` 归属判据 | ⭐ **必须与 U6 `OrderService.resolveLeader` 同判据**（缺陷⑫ 收口）：① 绑定且**在职** → 用；② 否则本楼 id 最小且在职 → 用；③ 都没有 → `null`。此前 U1 第一级**不看在职**，于是「团长停职 + 用户未重绑」时首页显示 A、佣金却记给 B（根因：D22「停职」只改 `ab_team_leader.status`、**不清** `ab_user.team_leader_id` —— 这是对的，复职后绑定该还在）。e2e `§M5-17` 用**停职**逼出分歧点，并以**独立复算**的 U6 判据对账（不复用 U1 结果，否则只能证明「U1 等于它自己」） |
...
| `supplier.id` | 出品方 id（`ab_supplier.id`）· **M5-17 新增**。用途单一：首页「来自：X」跳本页时带 `supplierId`，本页**按 id 定位**到那张卡并直接弹出平台层。按 id 而非 `name` —— `ab_supplier.name` 无唯一约束，同名两家会让用户**弹错店**；而用户在溯源页是照着弹出的店名去平台点单的，弹错家是这一页最坏的一类错误。⚠️ C8 不受影响：id 既不是合作状态，也不是供价 / 分账 / 联系方式 |
...
| **D7b** | **PUT** | **`/admin/meal/templates/{id}`** | **编辑模板**（2026-09-20 补 · 见下方口径） |
| — | GET | `/admin/meal/dishes?keyword=` | **扩展**：菜品选择器（D7 编排页候选菜品 · 只读） |
| — | GET | `/admin/meal/distribution-centers` | **扩展**：集散中心选择器（D2/D3 前置 · 只读） |

> **实现状态**：D1–D7 **已实现**（M3-2，`apps/api-server/src/modules/meal/meal-admin.*`），**D7b 已实现**（2026-09-20，同一服务），验收脚本 `scripts/e2e-m3.mjs` §14。
...
| D7 校验 | 先查**同菜重复档位**（同 `dishId` 占两档）→ **10001**；再查菜品不存在 / 已下架 → **30008**；最后查**一饭四菜构成** → **10001** | 顺序即口径：重复最廉价、构成最贵，先报便宜的错，运营改一次就过 |
| ⭐ **D7/D7b 一饭四菜（机械化）** | **必须恰好包含 4 个档位各一道**：`1 主荤` / `2 半荤` / `3 素菜` / `4 汤`；**不得出现第 5 档**（主食米饭由集散中心统一供，不建 `ab_set_meal_item`）—— 缺档 / 超档 → **10001**。真源 = `@abox/shared-utils` 的 `SET_MEAL_COMPOSITION`（`requiredSlots` / `stapleSlot` / `rule`），三处（API 校验 / 后台编排页 / 选择器出参 `composition`）同源 | 此前「一饭四菜」只活在文案与种子里：运营能建出「三道素菜」的套餐，用户端照卖。**约束不落成代码 = 约束不存在** |
| D7 售价缺省 | 未传 `price` → 回落 `ab_config.set_meal.default_price`（C1 锁定 ¥25.80） | 与 C1 统一定价同源 |
| 档位映射 | `slot`：1 主荤 / 2 半荤 / 3 素菜 / 4 汤 / 5 主食；**由 `GET /admin/meal/dishes` 出参下发**（`slots[].required` / `slots[].isStaple` 一并下发），标签真源 = `@abox/shared-utils` `SET_MEAL_SLOT_LABEL` | 端上不维护第二份映射，口径唯一在服务端 |
| 选择器 | `/admin/meal/dishes` **只回上架菜品**（`status=1`） | 选中已下架的菜，D7 提交时必被打回；不如根本不给选 |

**D7b 实现口径（2026-09-20 · 补编辑入口 · 勿推翻）**

| 主题 | 口径 | 为什么 |
| --- | --- | --- |
| 为什么要有 D7b | 此前只有 D7「存为模板」，模板建出来**只能改状态、不能改内容** —— 运营把套餐名字打错、或想换一道素菜，只能删库/新建，`ab_meal_assignment.set_meal_id` 的引用还会被连带影响 | 没有编辑入口的「模板库」是只写不改的，运营第一次用就会撞上 |
| ⭐ **引用冻结（fail-closed）** | `usedCount > 0`（被**未取消**的分配引用）时：**`items` 与 `price` 一律拒改 → 30019**（出参带 `usedCount` / `assignmentDates`）；`name` / `oneLiner` / `description` / `status` **仍可改** | 金额与出餐实物的来源被改动 ⇒ 已下单用户「买到的」与后台记的对不上。冻结**范围**本身就是口径：名字是给人看的，items/price 是给账和出餐看的 |
| 为什么不是「全锁」 | 全锁会让「把名字打错的模板」永远改不回来，运营只能另存为新模板 → 模板库迅速堆满近似重复项 | 该锁的锁、该放的放；`status` 上下架**不受引用冻结限制**（紧急下架是安全阀，同 D4 下架不设截单闸门） |
| `usedCount` 的语义 | = 被多少个**未取消**的分配引用（与 D6 列表同一口径，**不另立定义**） | 两页对「在用」的判据不同，就会出现「列表说在用、编辑说没在用」这类最难查的不一致 |
| 入参「不传 = 不改」 | 未出现的字段不动；显式传 `null` 才清空（`oneLiner` / `description` / `coverUrl`）；`items` 传了即**整组替换**（不是合并） | PUT 语义按「期望状态」而非「增量补丁」：合并会造出「删不掉菜」的模板 |
| D7b 校验 | 与 D7 **同一套**：重复档位 → **10001** → 不存在/下架 → **30008** → 一饭四菜构成 → **10001**；`items` 缺省时不重算成本 | 两个入口两条校验链，迟早分叉 |
| 成本重算 | `items` 变更则按菜品供价重算 `ab_set_meal.cost_price`（与 D7 同一函数） | 只改菜单不改成本 = 毛利看着很好、结算时对不上 |
| 审计 | 走 `@OperationLog({ module:'meal', action:'编辑套餐模板' })`，落 `ab_operation_log` | 模板改动影响面大（会被多天多楼群引用），须可回溯 |
...
| D66 | GET | `/admin/dashboard/todos` | **工作台待办聚合**（登录落点 `/dashboard` · M5-16 · 只读 · 实现口径见下方「M5-16 实现口径」表） |
...
**M5-16 实现口径（D66 工作台待办聚合 · 登录落点 · **零 DDL · 零新增错误码 · 零新状态**）**

| # | 方法 | 路径 | 说明 |
| --- | --- | --- | --- |
| **D66** | GET | **`/admin/dashboard/todos`** | **工作台待办聚合**：4 类待办计数 + 今日作业数字。只读、**只给计数不给明细**（明细在各域列表页） |

| 项 | 口径 |
| --- | --- |
| 为什么是**聚合端点**而不是前端各查一次列表 | 登录落点要回答的只有一个问题：「今天有什么必须我来处理的」。让前端各打一次列表接口再数条数会撞三件事：① 列表**分页**，拿不到总数；② 四次请求时序不同，卡片数字可能互相对不上（看到 3 条、点进去只有 2 条）；③ 前端得**复刻四份状态口径**（状态机在服务端，复刻必漂移）。**「同一件事有两份表述，不被执行的那份必然会错」** —— D66 就是为了消灭第 ③ 条而存在的 |
| ① `refundApplying` | `ab_refund.status='applying'`（C6 第二段的审批队列）· 处理页 `/finance/refund` |
| ② `withdrawPending` | `ab_withdraw.status='pending'`（D45 的审批队列 · L12 提交后的唯一推进点）· 处理页 `/finance/withdrawal` |
| ③ `tomorrowGroupsUnassigned` | 启用楼群数 − 明日有**「未取消分配」**的楼群数 · 处理页 `/meal/matrix` |
| ⭐⭐ ③ 的 `status != 'cancelled'` **不是可选项** | 它是本工程判「该楼群该日到底有没有套餐」的既有唯一判据（`meal.service.ts:72` · `meal-admin.service.ts` 模板 `usedCount`）。漏掉它，**停团（cancelled）的楼群会被算成「已排」** ⇒ 工作台显示已清空、而套餐矩阵里那格是空的 —— 两份口径打架且**两边都不报错** |
| ⭐⭐ ③ 只数「漏排」，**不数「排了没上架」（`pending`）** | `pending` 是**正常中间态**（编排可提前几天做，上架才是临近时的动作）。把它算成待办 ⇒ 开团（T-1 14:00）之前这张卡**永远非零**，一周内就会被学会无视。而「漏排」是硬缺口：不补，明日该楼群一定开不了团 |
| ④ `deliveriesLate` | 今日配送单 `status != 'arrived'`，**且仅在过了 `arrivalAtOf(今日)` 之后才计入** |
| ⭐⭐ ④ 送达时刻前**恒为 0**（且不查库） | 11:30 之前没送到**不是异常**（那正是送达时刻本身）。全天候报数会让这张卡每天上午都是红的 ⇒ 真正逾期的那天也没人看（同 `ops-daily.sh` 的「恒红的检查等于没有检查」）。故未到点连 `count` 都不发、直接短路为 0 |
| `brief`（今日作业数字） | `{ deliveryTotal, deliveryArrived, tomorrowGroupsTotal, tomorrowGroupsAssigned }` —— **与 `items` 同一批查询的副产物**，不额外查库；端上渲染「x / y 已送达」「x / y 楼群已排」 |
| `count = 0` 也下发 | 四项**恒返回 4 条**，不做「有才返回」的裁剪 —— 端上据此渲染「已清空」；少一条就再也分不清「今天确实没事」与「这一项忘了查」 |
| ⭐ 刻意**不含**供应商出餐进度 | `ab_supplier_dish_daily` 的 `pending/cooking/done` 是**实时进度盘**（供应商端 / 后台「加工场所打包」页），不是「必须由某个后台角色去批」的待办。放进工作台只会造出第二份进度口径，并在它滞后时制造假告警 |
| 权限（**含 `viewer`**） | 类级 `@Roles('super_admin','admin','operator','finance','viewer')`。`/dashboard` 是**五个运营角色的共同菜单项**（登录落点）—— 漏掉任何一个，该角色登录后的**第一屏**就全是 `10003`。本端点信息量严格**少于** D47 核心指标（后者已对 `viewer` 开放），不构成新暴露 |
| 端上仍按菜单过滤卡片 | 服务端对五个角色一律返回 4 条，但 `viewer` 点不进 `/finance/*`、`/meal/*`、`/order/*` ⇒ 端上按 `permission.visiblePaths` 滤掉点不开的卡。这是**渲染纪律**，不是安全边界（边界在各域控制器自己的 `@Roles`） |
| 双主体隔离 | 供应商 token 打 `/admin/dashboard/*` → `10003`；小程序 token → `10003`；未登录 → `10002`。⚠️ `/dashboard` 这个 key 在 `SUPPLIER_MENU_KEYS` 里**也有**，但那是**供应商侧的概览**（另一个页面、另一套菜单），不是「同一页开了两个角色」 |
| GET 不写操作日志 | 只读查询，**不标 `@OperationLog()`**（GET 打日志会把操作日志刷成垃圾） |
| 错误码 | **无新增**（复用 `10001` / `10002` / `10003`） |
| 落点 | `apps/api-server/src/modules/admin/dashboard/`（`dashboard.controller.ts` / `.service.ts` / `.module.ts`）· 页面 `apps/admin-web/src/views/dashboard/index.vue` · api `apps/admin-web/src/api/dashboard.ts`。⚠️ **单起 `DashboardModule`**（注册于 `app.module.ts`）而不是塞进 `AdminModule`：它是**跨域只读聚合**，既不是「后台自有的系统管理域」，也不属于任何单一业务域 —— 按 `StatsModule`（D47–D50 同样是跨域聚合）的先例单起，依赖面在一处写全 |
| e2e | 新增 **§38 · 20 条断言**：结构 5 条（返回 4 条 · key **顺序稳定** · 每条 count 非负整数 + `path` + `hint` · `businessDate`/`tomorrowDate` 同日口径 · `brief` 自洽）+ 四条计数**逐条对外域端点对账** 4 条（① ↔ D40 `refunds?tab=pending` ② ↔ D45 `withdrawals?status=pending` ③ ↔ D1 `stats.groupCount` ③b ↔ D1 `meal/matrix` 逐格数 ④ ↔ 送达时刻闸门：**自造夹具 + 翻转配置**双向验，闸门关必须恒 0、闸门开必须等于未送达条数，用完清理）+ 角色矩阵 6 条（operator / finance / viewer 一律 0 · 供应商 10003 · 小程序 token 10003 · 未登录 10002）。m3 断言 **1035 → 1055** |
...
| P27/P28/P29 | 套餐矩阵/新建/模板 | M31 | D1–D7、**D7b** | `ab_meal_assignment`、`ab_set_meal` |
...
| **30019** | 该套餐已排进某天的出餐计划，菜品与售价不能再改（可改名称，或另存为新模板） | D7b 编辑模板时已 `usedCount > 0`（被未取消的分配引用）· 出参 `data.usedCount` / `data.assignmentDates` —— **不能复用 30016**：30016 是「刷新重试即可」，这里是「重试一万次也不会成功」，该走「另存为新模板」。只冻结 **items / price**（金额与出餐实物的来源），`name` / `oneLiner` / `description` / `status` 仍可改 —— 冻结范围本身就是口径（**扩展**） | 409 |
