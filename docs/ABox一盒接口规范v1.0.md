...
| **U16** | `GET /me/agreements` | 一期未实装 · ⚠️ **提审前置** | 端上已按「不放点不动的条目」原则给如实说明；但**提审要求协议与隐私政策页面已上线、内容与实际收集项一致**（《提审自检清单 v1.2》第 11 条）⇒ **提审前必须补**（实装或改外链 H5） |
...
| U16 | GET | `/me/agreements?type=user\|privacy` | 用户协议 / 隐私政策正文（M04-06）· ⚠️ **一期未实装**（端上 `pages/mine/mine.vue` 已按「不放点不动的条目」原则给如实说明，不假装能点）—— ⚠️ **提审前置项**：《提审自检清单 v1.2》第 11 条要求「隐私政策 + 用户协议页面已上线、内容与实际收集项一致」⇒ **提审前必须补**（实装或改外链 H5） |
