...
| 最近修订（本轮 · 未实装端点标注） | 2026-09-20 · **契约「未实装」口径落成（纯文档 · 零代码改动 · 零新增端点 · 零 DDL）**：外部测试进场前，把「规范里有行、代码里没有」的缺口逐条落成**明示契约**。① ⭐⭐ **新增 §1.8「本期未实装端点清单（对外测试必读）」** —— 14 条逐条给「性质 + 为什么不实装 + 何时补」；其中 **U12 是刻意不设端点（由 A2 承担）、不是缺口**，其余 13 条为一期未实装。② ⭐⭐ **判据可复算且经交叉验证**：仓库内既有门禁 `route:audit`（**NestJS 运行时反射**）实测 141 端点；另在工作区根新增 `tests/tools/audit-contract.mjs`（**静态**解析 controller 装饰器 × 本文件端点表）做对账 —— 两法端点集合**完全相等**（141 = 141 · 交集 141 · 双向差额 **0**），**两个独立取数方法一致**才算数。③ ⭐ **豁免会自收紧**：对账工具认「行内显式声明」为已声明（**不静默忽略** —— 静默忽略比漏报更快把人训练成忽略告警），把声明删掉即重新变红。④ ⭐ §五 节首加一次性提示（10 条未实装讲清，省得测试人逐条踩）。⑤ ⚠️ **顺手更正一处自相矛盾**：§1.6「图片」行原写上传走后 `POST /me/upload-token`，而 §3.1 **U4 定义为 `GET`** —— 同一端点两种方法、两处都不报错，已更正为 U4 `GET` 并标注未实装。⑥ ⚠️ **不改任何代码**：本轮只把**既有事实**写进契约，不新增/不修改端点，`gate.mjs` 22/22 不受影响。 |
...
>    —— 实测 **31 个 `*.controller.ts` → 141 个端点**；
> ② 对账侧：工作区根 `tests/tools/audit-contract.mjs`（**静态**解析 controller 装饰器 × 本文件端点表），
>    两法端点集合**完全相等**（141 = 141 · 交集 141 · 双向差额 0）⇒ 下方结论可采信。
