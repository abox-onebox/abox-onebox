...
### 8.1 后续处置（2026-09-18 当日回填）

| 条目 | 裁决 | 状态 |
| --- | --- | --- |
| **#79** 履约链 T7–T9 零写入点 | 用户裁定「**补实现**」 | ✅ **已收口（M5-8）**。T7 落在 `SupplierService.advanceOrdersToCooked`（出餐确认同事务联动，判据与「打包 ready」同源）；T8/T9 落在**新增端点 D63** `PATCH /admin/deliveries/{id}/status`；D61 列表另新增 `orderStatusBreakdown`（履约可见性）。**佣金链由此打通**（`accrueForOrders` 的两个调用点不再不可达）。详见《接口口径》§M5-8 与《缺陷与陷阱》#79 的更新段。 |
| **#84** 「声明 ↔ 写入点」无机械对账 | 随 #79 一并处置 | ✅ **门禁收紧**：`state:audit` 豁免表 **4 → 0**，新增规则 ④「不可达态必须显式登记」，并**删掉 `refund_applying → refunding` 那条躺了三个批次的不可达边**。⚠️ 门禁经 **5/5 反证探针**打红过。 |
| ⭐ **#79 留下的可复用判据**（比修法本身更值钱） | — | **「夹具直写 DB」是有代价的** —— 它让「上游缺失」在测试里**不可见**。**凡是用「直插某状态」造数据的套件，都必须额外有一条走完整链路的用例**，否则它只证明了「后半段能跑」，而不是「这条路能走通」。本轮落地为 e2e **§35**（27 条断言 · 真实 HTTP 全链路）。 |
| **#86** D63 单向性**放行了跳级**（注释/文档三处都写「一律拒」） | M5-8 **实现自查当场盘出** | ✅ **已修**：`allowed` 改为**只含紧邻下一态**（终态 `[]`），三处文字与代码对齐；e2e §35 新增 `called → arrived` → `30018` 且 `allowed === ["en_route"]`。⚠️ 跳级的真实后果不是「少点两次」，而是**跳过 T8** ⇒ 订单还在 `cooked` 而 T9 只改 `delivering` ⇒ **一单不动且不报错**（#79 的形状，不能在刚补好它的同一批里重开）。详见《缺陷与陷阱》#86。 |
| ⭐ **#86 的附带判据：「接口有、入口无」** | — | D63 起初只有端点、后台配送页**没有入口** ⇒ 能力存在但**无人能到达**（与 #79 同族）。本批补「推进到 X」按钮 + 确认弹窗 + 「订单进度」列。**验收标准是「测试人照着清单能点到」，不是「e2e 能过」** —— e2e 走 HTTP，看不见「界面里没有这个按钮」。⚠️ 同族未收口项：`/admin/schedule`（时刻表 + 手动补跑）**只有端点、无后台页面**，手工测试触发跑批目前只能靠 curl 或等 cron。 |
| ⭐ **反证纪律的再一次兑现** | — | 本轮新写的规则 ④ 与自证 ④ 都被反证打红过；且**反证脚本自己出过一次错**（P5 探针「以为在攻击、其实没攻击到」）—— 见《缺陷与陷阱》#84 更新段。**任何新写的检查器，在被反证过一次之前，它的「绿」不构成任何证据。** |
| 其余条目（#80–#83 / #77） | M5-7 已收口本地可验部分 | 见《接口口径》§M5-7 与《缺陷与陷阱》#80–#83。⚠️ **真机 MySQL 并发验证仍挂账**（`better-sqlite3` 单连接串行化，本地任何「绿」都不构成并发证据）。 |
