...
│   ├── gate.mjs                        ← 免 pnpm 门禁执行器（⭐ **20 道**：M5-2 增 `schema:parity`，M5-3 增 `route:audit` + `security:scan`，M5-6 增 `index:parity`，M5-7 增 `state:audit`）
...
│   ├── lib/local-server.mjs            ← 【本地内部测试】静态托管 + `/api` **同源反向代理**（手机访问不依赖固定 IP、无跨域）
...
│   │   │   ├── order-state-audit.ts    ← 【M5-7 增】⭐ **状态机声明 ↔ 生产写入点**机械对账（`gate.mjs` 门禁 `state:audit`）—— 抓「声明了迁移目标但全仓零写入」这类**不报错的 P0**（#79）；含**显式豁免表**（未实装项，条目带理由+账本引用，被实现后门禁要求删除）+ 三重自证
│   │   │   └── dto/
...
*文档结束 · ABox 一盒 · 项目目录结构 v2.0（现行 v2.0.11） · 2026-09-17*
