...
│   │   │   └── leader-invite.vue
│   │   ├── traceability/               ← P38 供应商（原型页名「今日这盒 · 商家溯源」· C8 · **M5-14 实装**）
│   │   │   └── traceability.vue        ← M5-14 由占位页改实装（溯源说明卡 + 逐菜出品方卡 + 三平台跳转面板）
...
│   │   ├── ab-meal-card/
│   │   ├── ab-dish-card/
│   │   ├── ab-supplier-card/           ← 【M5-14 实装】溯源出品方卡片（C8 · 原型 P38）—— 一组件两形态：供应商卡（带三平台角标）/ 集散中心卡（带地址）
│   │   ├── ab-status-badge/
│   │   ├── ab-countdown/
│   │   ├── ab-empty-state/
│   │   ├── ab-loading/
│   │   └── ab-bottom-bar/              ← 底部导航（M5-14：第 2 个 tab 由「溯源」改名「**供应商**」以对齐原型 P38）
...
│   │   ├── order.ts
│   │   ├── leader.ts                   ← 团长端（含代退申请）
│   │   ├── traceability.ts             ← 【M5-14 实装】今日这盒 / 出品方（U5 · `auth:false` 免登录只读）
│   │   ├── payment.ts
│   │   └── balance.ts
│   ├── utils/
│   │   ├── auth.ts                     ← 登录态管理
...
│   │   │   ├── response.ts
│   │   │   └── takeout.ts              ← ⭐⭐ M5-14 `ab_supplier.takeout_links` 的**唯一读取口**
│   │   │                                  （后台 D33 配置页与用户端 U5 溯源**共用**）。吸收两个落库坑：
│   │   │                                  ① 键缺失 ≠ 未入驻（三平台恒返回 3 条，`configured=false`）；
│   │   │                                  ② `__recommended.url` 存的是**平台 key 而非链接**。
│   │   │                                  并做**读侧兜底**：悬空推荐（推荐了但该平台未配置）⇒ `null`
...
│   │   │   ├── meal.service.ts
│   │   │   └── dto/
│   │   ├── traceability/               ← ⭐⭐ **M5-14 实装**（此前 `controller` 是**零路由空类**、`module` 是空模块）今日这盒 · 出品方溯源（C8 · 只读聚合 · `@Public()` 免登录）
│   │   │   ├── traceability.module.ts
│   │   │   ├── traceability.controller.ts ← **U5** `GET /traceability/today`（`buildingId` 必传 —— 免登录下守卫不注入 `req.user`，无从推楼群）
│   │   │   ├── traceability.service.ts    ← 按 `buildingId → buildingGroupId` 定位 `ab_meal_assignment` → 逐菜列出品方；**N+1 收敛**为固定 4 次查询（供应商视图按 id 缓存，与菜数无关）；`supplierView()` 是 **C8 收口点**（只出 名称/资质/推荐/三平台）
│   │   │   └── dto/                       ← 【M5-14 新增】`TraceabilityTodayQueryDto`
│   │   ├── order/                      ← 订单
│   │   │   ├── order.module.ts
│   │   │   ├── order.controller.ts
│   │   │   ├── order.service.ts
...
├── tsconfig.json
├── src/
│   ├── index.ts                        ← 统一出口（`export * from './dto/…'` + `'./enums/…'`）
│   ├── api/
│   │   └── common.ts                   ← 统一响应结构与分页壳
│   ├── dto/                            ← 共享 DTO（前后端共用 · ⚠️ 旧文档曾把它们记在 `api/` 下，实际在 `dto/`，M5-14 更正）
│   │   ├── create-order.dto.ts
│   │   ├── home.dto.ts                 ← U1 首页泳道
│   │   ├── order.dto.ts
│   │   ├── payment.dto.ts
│   │   └── traceability.dto.ts         ← 【M5-14 新增】U5 溯源契约（`TraceabilityTodayResult` / `TraceabilitySupplierView` / `TraceabilityQualification`）
│   ├── enums/                          ← ⚠️ 以**实际文件**为准（旧文档含已不存在的 `meal-status.ts`，M5-14 更正）
│   │   ├── order-status.ts             ← 用户 5 态 / 团长 6 态 / 后台 8 态
│   │   ├── refund.ts                   ← 退款状态（C6 三段式）
│   │   ├── withdraw-status.ts          ← 提现单五态
│   │   ├── delivery-status.ts          ← 配送单履约状态（**M5-1** 收敛：原在 `leader-order.service` 与 `workbench.service` 各写一份）
│   │   ├── role.ts
│   │   ├── leader-level.ts             ← 见习 / 正式 / 金牌 / 首席（**M5-13** 起费率真源改为 `ab_config`，此处只作出厂兜底）
│   │   ├── leader-status.ts            ← 团长 1 在职 / 2 停职
│   │   ├── leader-admin.ts             ← D19–D22 后台团长管理枚举
...
| **P38** | **今日这盒 · 商家溯源**（端上入口名 = **「供应商」**） | **M01-04** | **`pages/traceability/`**（⭐ **M5-14 实装** · 此前是占位页） |
...
| **v2.0.22** | 2026-09-19 | **M5-14 溯源实装 + 底部 tab 改名「供应商」（收口《全面检查报告》P1-5 · 零 DDL · 零新增错误码 · 门禁仍 20 道 · 表数仍 27）**：① §二 `pages/traceability/` 由「占位两行」改**实装**（`traceability.vue` = 溯源说明卡 + 逐菜出品方卡 + 三平台跳转面板）；⚠️ **删掉一行不存在的东西** —— 原登记的 `pages/traceability/components/supplier-card.vue` **全仓零引用**，是脚手架残留，且内含硬编码「资质三重核验」徽章 —— 与 U5 实装**刻意不输出健康证**的口径**正好相反**，留着就是一句躺在仓库里的**虚假合规声明**；已删除并在 §二 就地说明；② §二 `components/` **整块更正** —— 8 个组件实际目录名**一律带 `ab-` 前缀**（`ab-meal-card` / `ab-dish-card` / `ab-supplier-card` / `ab-status-badge` / `ab-countdown` / `ab-empty-state` / `ab-loading` / `ab-bottom-bar`），旧文档**一个前缀都没写** ⇒ 照文档去找必然找不到；`ab-bottom-bar` 就地标注 **M5-14 第 2 个 tab 由「溯源」改名「供应商」**以对齐原型 P38；③ §二 `api/traceability.ts` 标注 **M5-14 实装**（`auth:false` 免登录只读）；④ §四 `common/utils/` 补 **`takeout.ts`**（⭐⭐ `ab_supplier.takeout_links` 的**唯一读取口** —— 后台 D33 与用户端 U5**共用**，吸收「键缺失 ≠ 未入驻」与「`__recommended.url` 存的是平台 key 而非链接」两坑，并做悬空推荐读侧兜底）+ 补登**长期漏列的** `money.ts` / `password.ts`；⑤ §四 `modules/traceability/` 由「三行无注释」改**实装登记**（`controller` 此前是**零路由空类** · `service` 此前是空文件 · 新增 `dto/`），就地写明 **C8 收口点**是 `supplierView()`、以及「N+1 收敛为固定 4 次查询」；⑥ §五 `packages/shared-types` **整节按实际树重写** —— ⚠️ 原文把 DTO 记在 `api/` 下并虚列 `user.ts` / `meal.ts` / `order.ts` / `delivery.ts` / `team-leader.ts` / `supplier.ts` / `distribution-center.ts` / `traceability.ts` / `finance.ts` 九个文件，**一个都不存在**（真实位置是 `dto/`），`enums/` 里记的 `meal-status.ts` 同样不存在；现 `dto/` 补 `traceability.dto.ts`（**【M5-14 新增】**）与 `home.dto.ts`、`enums/` 补齐 12 个真实文件；⑦ §十 **10.1** P38 行标注实装 + 端上入口名「供应商」；⑧ ⚠️ **本节历史欠账如实保留**（M4-0…M4-4 各批次未逐批登记）；⑨ ⚠️ 另登记一条**门禁覆盖盲区**：文档描述与真实树是否一致，**没有任何门禁会发现**（`dead-files-scan` 只扫源码、且只认「空导出 + 零引用」——本轮这个**带实现的死文件**与**虚列路径**都不在其判据内） |
| **v2.0.21** | 2026-09-19 | **M5-13 两处「两个真相」收口 + 结构对账补尺寸维度（零 DDL · 零新增错误码 · 门禁仍 20 道 · 表数仍 27 · **零新增文件**）**：① §四 `database/schema-parity.ts` 登记补齐 —— 由「结构 + 列名/类型」扩到**尺寸维度**：三类**分别落位**（`VARCHAR/CHAR(n)` → 长度 · `DECIMAL/NUMERIC(p,s)` → 精度 + 标度 · `DATETIME/TIMESTAMP/TIME(n)` → 小数秒精度，TypeORM 放在 `precision` 上），⚠️ 混为一谈会让 **84 个 `DATETIME(3)` 全假红**；取数侧改读 **TypeORM 实体元数据**（`ColumnMetadata.length/precision/scale`），**刻意不读 sqlite 读回列**（后者对 `VARCHAR` 恒为空串 = 一道**恒不报错**的检查）；⚠️ 原 `COLUMN_DEF_RE` 是单行正则 + `split(',')` 切段，`DECIMAL(12,2)` 会被**切碎、精度静默丢光** —— 这正是该门禁此前自述「不比长度 / 精度」的**隐蔽原因**；② 实测 **27 表 / 394 列 → 287 个尺寸维度两侧都有声明且全部相等**、单侧声明 **0** 项；`selfTest` 扩 ③ 分派自证 + ③b **未登记类型族必须抛错**（fail-closed）+ ④ **真实数据深拷贝改尺寸必须报红**（`VARCHAR(32)→(64)`、`DECIMAL(12,2)→(12,3)` 均 exit 1）；③ §四 `modules/team-leader/` 登记「**等级 → 费率**」消费点收敛 —— 收口前是 **2 个来源 × 4 个消费点**（只有 `promotion.service` 读 `ab_config.commission.rate.*`，另三条路读常量 `LEADER_LEVEL_META[level].rate`；而 `config.specs.ts` 把这四键声明为 `wiring: 'live'` ⇒ **声明与实现不一致、两侧都不报错**）；现统一为 `BizConfigService.commissionRate()` **唯一口**（`level.service.ts` `rules()` / `leader-admin.service.ts` `rateOf()` 与 `levelOptions()` / `team-leader.service.ts` L17 建档）；④ `scripts/e2e-m3.mjs` 增 **§37**（9 条断言 · m3 **1017 → 1026**）：改配置 ⇒ 三条读路径 + 一条写路径同步变、**改回 ⇒ 全部回原值**（单向断言抓不到「读一次就缓存成常量」）、**夹具逐键复原并断言复原**；⚠️ 本节**必须排在 §34 之前**（§34 会停主线实例另起一台开限流的服务）；⑤ ⚠️ **两条已登记陷阱本轮各复现一次**：**直跑 `e2e-m3.mjs` 不带 `seed`** → 崩于 `TypeError: Provided value cannot be bound to SQLite parameter 1`（**看起来像套件坏了**）· **`gate.mjs` 成功时吞掉子进程 stdout** → 断言总数只能**复刻 gate 的 env 直跑**才取到（`E2E_PORT=3103` + `ABOX_SHIFT_TO_HOUR=20` + `QUEUE_DRIVER=memory` + `QUEUE_BACKOFF_BASE_MS=20`）；⑥ 《模拟数据与回归测试结果》新增**附四**（含 §37 九条判据表 + 两件可复用的踩坑）·《开发里程碑计划》升 **v1.1.23**（§五 进度块由「13 个子批次」改为 **14 个（M5-0 ~ M5-13）** + 新增 M5-13 子批次段 + 5.1 断言数 **1017 → 1026**）；⚠️ 未入册的历史欠账（M4-0…M4-4 各批次未逐批登记）仍未清 |
