...
| 现行资产 | 29 份 |
| 已废弃资产 | 7 份（存档，勿参考） |
| 工程骨架 | `abox-onebox/` · **493 个文件**（阶段四产出） |
...
| 基线 commit | 基线 tag `v1.1-local-dev-base`（`0e9552e` · 388 文件）· 生成时 HEAD `409f7e3`（**回溯基准**：清单是生成物，其自身提交号 = 上述 HEAD 的下一笔 `chore(baseline)` 提交） |
...
| 6 | `tests/baseline_manifest.py` | 本清单生成器（基线变更时重跑） | 74,389 | `c5a0a7aa49c4` |
