...
| 现行资产 | 29 份 |
| 已废弃资产 | 7 份（存档，勿参考） |
| 工程骨架 | `abox-onebox/` · **505 个文件**（阶段四产出） |
...
| 基线 commit | 基线 tag `v1.1-local-dev-base`（`0e9552e` · 388 文件）· 生成时 HEAD `0e6b1a9`（**回溯基准**：清单是生成物，其自身提交号 = 上述 HEAD 的下一笔 `chore(baseline)` 提交） |
...
| 15 | `ABox一盒开发里程碑计划v1.0.md` | M1–M5 里程碑 + W1–W10 甘特 + 验收标准 + 风险登记册（阶段四） | 97,521 | `bd7e47fd825a` |
| 16 | `ABox一盒接口规范v1.0.md` | 接口契约（阶段二）· 60+ 端点 / 错误码 / 幂等 · 2026-09-16 S9 应付结算改「采购应付」口径 | 224,371 | `20bce172c939` |
...
| 25 | `ABox一盒项目目录结构v2.0.md` | Monorepo 布局 · 端页模块映射 · v2.0.2 补 leader-expire.task | 36,065 | `2bbb9a1df652` |
...
| 187 | `apps/api-server/src/config/app.config.ts` |  | 4,444 | `2d5fc09bf67b` |
...
| 6 | `tests/baseline_manifest.py` | 本清单生成器（基线变更时重跑） | 115,311 | `f06c4b474482` |
