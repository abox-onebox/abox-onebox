...
| 现行资产 | 30 份 |
| 已废弃资产 | 7 份（存档，勿参考） |
| 工程骨架 | `abox-onebox/` · **518 个文件**（阶段四产出） |
...
| 15 | `ABox一盒开发里程碑计划v1.0.md` | M1–M5 里程碑 + W1–W10 甘特 + 验收标准 + 风险登记册（阶段四）· ⭐ v1.1.12（2026-09-17）M5 段**三个子批次**：**M5-0 部署运维基座** + **M5-1 配送单人工修正**（D61/D62 · 收口挂账 #61）+ **M5-2 生产迁移补齐 + 结构对账门禁**（收口 #76 · 补 2 表 9 列 1 索引 + `schema:parity`）；M5 段标注 4 项**外部条件阻塞**（5.2 压测 / 5.4 真机部署 / 5.5 灰度 / 5.6 演练）（⚠️ 子批次是前置 **≠ M5**；验收标准 4 条**全部要求真机执行**，三个子批次的完成**均不构成**对它们的满足） | 123,934 | `b4fa3de149a1` |
| 16 | `ABox一盒接口规范v1.0.md` | 接口契约（阶段二）· 60+ 端点 / 错误码 / 幂等 · 2026-09-17 **M5-1 加 D61 `GET /admin/deliveries` + D62 `PUT /admin/deliveries/{id}`（配送单人工修正）+ 错误码 30016/30017** · 2026-09-16 S9 应付结算改「采购应付」口径 | 240,007 | `3ac91c8c9d08` |
...
| 22 | `ABox一盒订单状态机与全链路流转v1.0.md` | 订单域行为契约（阶段二）· 11 态 / 8 定时任务 / C6 退款三段式 / C11 出款通道 | 31,967 | `dc0d6fd86d63` |
...
| 15 | `scripts/gate.mjs` |  | 12,694 | `b0d67e22dbeb` |
...
| 68 | `scripts/e2e-m3.mjs` |  | 594,509 | `7d88ed336840` |
...
| 131 | `apps/api-server/src/modules/admin/config/config.specs.ts` |  | 21,579 | `afdd3d5e37a8` |
...
| 190 | `apps/api-server/src/tasks/schedule.service.ts` |  | 12,040 | `b5d4049b8aa5` |
| 191 | `apps/api-server/src/tasks/schedule-admin.controller.ts` |  | 3,837 | `6a9904e9799d` |
| 192 | `apps/api-server/src/tasks/schedule-admin.service.ts` |  | 8,595 | `a6fa0b039ca9` |
| 193 | `apps/api-server/src/tasks/dto/schedule.dto.ts` |  | 1,072 | `ef34a7a8ff5e` |
| 194 | `apps/api-server/src/tasks/meal-publish.task.ts` |  | 3,136 | `4ebde7bcdfad` |
| 195 | `apps/api-server/src/tasks/cutoff.task.ts` |  | 4,391 | `491c0d247b1e` |
| 196 | `apps/api-server/src/tasks/delivery-generate.task.ts` |  | 2,403 | `9cb437a8c7ba` |
| 197 | `apps/api-server/src/tasks/auto-confirm.task.ts` |  | 4,200 | `6d8c6b312259` |
| 198 | `apps/api-server/src/tasks/commission-settle.task.ts` |  | 4,395 | `4a9aee3adff4` |
| 199 | `apps/api-server/src/tasks/reconciliation.task.ts` |  | 7,321 | `bfc866698e97` |
...
| 6 | `tests/baseline_manifest.py` | 本清单生成器（基线变更时重跑） | 140,878 | `030d0c4a4e7a` |
| 7 | `tests/tools/gate.mjs` | ⚠️ **历史版本（12 道）· 已不是现行门禁执行器** —— 现行的是仓库内 `abox-onebox/scripts/gate.mjs`（M5-2 后 **16 道**，含 `schema:parity`）。此件保留仅为基线哈希可复核，**勿据它判断门禁道数或门禁名** | 7,126 | `dc39ff6c2773` |
