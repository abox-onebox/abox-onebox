...
| 现行资产 | 30 份 |
| 已废弃资产 | 7 份（存档，勿参考） |
| 工程骨架 | `abox-onebox/` · **514 个文件**（阶段四产出） |
...
| 基线 commit | 基线 tag `v1.1-local-dev-base`（`0e9552e` · 388 文件）· 生成时 HEAD `91cba69`（**回溯基准**：清单是生成物，其自身提交号 = 上述 HEAD 的下一笔 `chore(baseline)` 提交） |
...
| 15 | `ABox一盒开发里程碑计划v1.0.md` | M1–M5 里程碑 + W1–W10 甘特 + 验收标准 + 风险登记册（阶段四）· v1.1.10（2026-09-17）M5 段标注：4 项**外部条件阻塞**（5.2 压测 / 5.4 真机部署 / 5.5 灰度 / 5.6 演练）+ 新增子批次 **M5-0 部署运维基座**（⚠️ M5-0 是前置 **≠ M5**；验收标准 4 条**全部要求真机执行**） | 107,887 | `46403627b5de` |
| 16 | `ABox一盒接口规范v1.0.md` | 接口契约（阶段二）· 60+ 端点 / 错误码 / 幂等 · 2026-09-16 S9 应付结算改「采购应付」口径 | 228,423 | `7fd3cf461e94` |
...
| 26 | `ABox一盒项目目录结构v2.0.md` | Monorepo 布局 · 端页模块映射 · v2.0.5（2026-09-17 M5-0 部署落点：`docker-compose.prod.yml` / 双 Dockerfile / nginx.conf / `.dockerignore` / 五个运维脚本；§一 `scripts/` 按**实际文件**重写 —— 原列的 `db-migrate.sh` / `db-seed.sh` 从未存在）· v2.0.2 补 leader-expire.task | 40,624 | `ccd387fd1396` |
...
| 27 | `apps/admin-web/src/constants/index.ts` |  | 7,548 | `033b653248db` |
...
| 50 | `apps/api-server/src/common/constants/admin-role.ts` |  | 6,999 | `fd878b86667b` |
...
| 53 | `apps/api-server/src/common/constants/error-code.ts` |  | 18,327 | `5a680f86bf82` |
...
| 68 | `scripts/e2e-m3.mjs` |  | 565,329 | `1a454b8e8c40` |
...
| 121 | `apps/admin-web/src/router/routes.ts` |  | 9,211 | `8429b2a9565d` |
...
| 6 | `tests/baseline_manifest.py` | 本清单生成器（基线变更时重跑） | 124,938 | `c57bb54ddeaf` |
